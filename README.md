...
- [x] 自 2026-04-06 起，本机 GitHub 访问与推送需与电脑当前代理口径保持一致；当前已确认可用的本地代理端口为 `127.0.0.1:7890`。若 `git push` 长时间无返回，优先检查是否通过该代理端口访问 GitHub。（确认日期：2026-04-06）
...
- 当前结论：
  ...
  - 前端可验证入口补记（确认日期：2026-04-05）：
    ...
    - 已继续完成第八轮结果卡拆分：在 [cards.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/openclaw-playground/cards.tsx) 中新增独立的成功结果卡、只读拦截卡、失败结果卡和初始空态卡；[sections.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/openclaw-playground/sections.tsx) 中的结果区现已只负责路由到不同卡片组件。这样后续若继续优化成功摘要、只读拦截说明或失败态排障提示，可由不同 agent 按卡片边界并行修改，不再集中冲突在一个结果区 JSX 块中。
    - 本轮已再次完成本地验证：`frontend npm run build` 继续通过；当前仍只有既有的 Vite chunk size 警告，没有新增类型或构建错误。
    - 已继续完成第九轮性能收口：在 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 中将飞书集成、OpenClaw 联调、图标/Logo插件库改为 `React.lazy + Suspense` 按需加载，并补充统一的加载占位卡片。当前这些二级大模块不再跟随系统设置首页一起进入主包。
    - 本轮构建结果已确认拆包生效：构建产物中已新增独立 chunk `FeishuIntegrationView-*.js`、`OpenClawPlaygroundView-*.js`、`PluginLibraryView-*.js`；最大主 chunk 已从上一轮约 `1542 kB` 降到约 `1467 kB`，说明这轮按需加载已经对主包收缩产生直接效果。当前仍存在更大的剩余 chunk，下一步应继续考虑 `App.tsx` 级别的页面懒加载或 `manualChunks` 策略。
    - 已继续完成第十轮页面级懒加载：在 [App.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/App.tsx) 中对项目、商机、方案、投标、合同、知识库、数据分析、系统设置、帮助与支持等页面启用 `React.lazy + Suspense`，只保留登录页和默认工作台作为静态首屏视图；同时补充统一的页面级加载占位卡片，避免菜单切换到懒加载页面时出现空白闪烁。
    - 本轮构建结果已进一步确认页面级拆包生效：构建产物中已新增独立 chunk `ProjectsView-*.js`、`SolutionsView-*.js`、`BidsView-*.js`、`ContractsView-*.js`、`KnowledgeView-*.js`、`AnalyticsView-*.js`、`SettingsView-*.js`、`OpportunitiesDemoView-*.js` 等。最大主 chunk 已进一步从约 `1467 kB` 降到约 `1047 kB`，说明页面级懒加载已经显著缩小主入口包体积。当前剩余优化重点已经从“页面是否拆开”转向“公共依赖如何继续分桶”，下一步更适合评估 `manualChunks` 策略。
    - 已继续完成第十一轮公共依赖拆包：在 [vite.config.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/vite.config.ts) 中落地首版 `manualChunks` 策略，将 `react/react-dom`、`antd/@ant-design/icons`、`echarts + zrender`、`echarts-gl` 与其余公共依赖拆分为独立 vendor chunk。中途尝试过把 `rc-*` 单独拆桶，但会引入循环 chunk 警告，因此已回退到更稳妥的分桶方案，当前构建无新增异常警告。
    - 本轮构建结果已确认进一步拆包生效：产物中已新增 `vendor-react-*.js`、`vendor-antd-*.js`、`vendor-echarts-core-*.js`、`vendor-echarts-gl-*.js`、`vendor-misc-*.js`。其中 `echarts-gl` 已从图表主包中剥离为独立 chunk；当前剩余较大的公共块主要是 `vendor-echarts-core`、`vendor-misc`、`vendor-antd`，说明下一步应继续基于依赖图而不是页面边界继续做更细的 vendor 分桶。
    - 已继续完成第十二轮 vendor 微调：在 [vite.config.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/vite.config.ts) 中继续将 `@emotion/stylis` 独立为 `vendor-emotion`，并把 `claygl` 并入 `vendor-echarts-gl`。中途尝试过继续拆 `rc-*` 和 `dayjs`，但会引入循环 chunk 或空 chunk，因此已回退，保留当前这版更稳妥的分桶结果。
    - 本轮构建结果已确认：`vendor-misc` 已进一步缩小到约 `411 kB`；`vendor-echarts-gl` 在合并 `claygl` 后约 `600 kB`，但由于它只随图表相关页面按需加载，整体风险低于让这些依赖留在通用杂项包中。当前下一步的重点已经非常明确：继续评估 `vendor-echarts-core` 是否还能按图表能力拆分，或继续下沉到业务组件级动态导入。
    - 已继续完成第十三轮图表能力级收口：在 [EChartsPreview.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/components/EChartsPreview.tsx) 中，将原先整包 `import("echarts")` 调整为 `echarts/core` + 明确注册 `bar / line / pie / scatter / graph / gauge / sankey` 等当前页面实际使用的 2D 图表与所需组件/渲染器；3D 图表仍继续按需加载 `echarts-gl`，从而兼顾 2D 主体收缩和 3D 场景兼容性。
    - 本轮构建结果已确认：`vendor-echarts-core` 进一步从约 `1041.77 kB` 下降到约 `1023.22 kB`；虽然降幅有限，但该改动已经把“整包动态引入”替换为“按已知图表能力注册”的更可控结构，为下一步继续在 `AnalyticsView` 中按 2D/3D 图表场景做更细懒加载创造了前提。
...
- 本次会话在“前端性能收口 + OpenClaw 联调页可维护性增强”这一主线上先暂停，不再继续拆包或继续拆组件，作为明日继续的稳定停点。
- 当前本地与 GitHub 分支 `docs/feishu-openclaw-dev-plan` 已同步到最新检查点，最近一次提交为 `115b2d2`：`Refine ECharts preview core module loading`。
- 当前已确认的本地验证基线：
  - `frontend npm run build` 通过；
  - 前端页面级懒加载、系统设置子模块懒加载、OpenClaw 联调页组件拆分、结果卡拆分、状态矩阵文案收口均已落地；
  - `vite.config.ts` 中的 vendor 拆包策略当前以“稳定优先”为准，已避免保留循环 chunk 或空 chunk 方案。
- 当前前端包体收口结果：
  - 页面级懒加载已完成；
  - 首版 `manualChunks` 已完成；
  - `vendor-misc` 已压到约 `411 kB`；
  - `vendor-echarts-core` 约 `1023 kB`，仍是下一步最值得继续处理的重点。
- 明日若继续，建议优先顺序如下：
  1. 继续评估 `AnalyticsView` 中 2D/3D 图表的更细粒度懒加载边界，重点压缩 `vendor-echarts-core`；
  2. 若暂不继续性能线，则切回 OpenClaw 联调页，进入真实联调验收和交互细节收口；
  3. 所有后续较大前端改动，仍保持“先本地验证，再推 GitHub 检查点”的节奏。

### 最新进展补充（2026-04-07）

- 当前处理的任务：按上一轮停点继续执行“前端性能收口”，优先压缩 `AnalyticsView` 依赖的 `vendor-echarts-core`。
- 本次新增内容（本地代码）：
  - 已先将 [frontend/src/components/EChartsPreview.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/components/EChartsPreview.tsx) 从“运行时动态整包导入 `echarts/charts` 与 `echarts/components`”改为“明确模块导入”，完成第一轮核心收口；
  - 随后继续把 `EChartsPreview` 改造成“基础组件常驻 + 图表模块按 `moduleKeys` 异步 install”模式：`bar / line / pie / scatter / effectScatter / graph / gauge / sankey / geo` 均按实际面板需要延迟注册，不再在标准销售仪表盘首屏统一同步装配；
  - 已在 [frontend/src/views/AnalyticsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/AnalyticsView.tsx) 为默认 4 张面板和设计器预览显式标注图表依赖，例如销售漏斗只加载 `bar`、业绩趋势加载 `bar + line`、行业分布加载 `pie`、`china_map` 加载 `effectScatter + geo`；
  - 已新增懒加载子模块 [frontend/src/views/analytics/AnalyticsDashboardDesigner.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/analytics/AnalyticsDashboardDesigner.tsx)，并在 [frontend/src/views/AnalyticsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/AnalyticsView.tsx) 中通过 `React.lazy + Suspense` 接管“自定义仪表盘设计器”整块编辑区，使标准销售仪表盘首屏不再同步装入设计器 JSX；
  - 已继续新增懒加载子模块 [frontend/src/views/analytics/AnalyticsDashboardPreview.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/analytics/AnalyticsDashboardPreview.tsx)，并在 [frontend/src/views/AnalyticsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/AnalyticsView.tsx) 中通过 `React.lazy + Suspense` 接管“全屏大屏预览层”，使标准销售仪表盘首屏不再同步装入预览弹层的大块 JSX；
  - `echarts-gl` 仍保持为可选异步加载，不与本轮 2D 核心收口混在一起，继续维持 3D 图表按需拉起的口径；
  - 中途尝试过在 [frontend/vite.config.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/vite.config.ts) 继续细分 `vendor-echarts-standard / vendor-echarts-extended`，但会引入循环 chunk 警告；该方案已回退，当前仍保持“稳定优先”的分桶策略。
- 本次验证结果：
  - 已执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建通过；
  - 第一轮构建产物显示 `vendor-echarts-core` 已从上一轮记录的约 `1023 kB` 下降到约 `683.12 kB`；
  - 在继续启用“按模块异步 install”后，最终稳定结果又进一步下降到约 `647.22 kB`；
  - 设计器懒加载接入后，产物中已新增独立 chunk `AnalyticsDashboardDesigner-*.js`；同时 `AnalyticsView-*.js` 主 chunk 已进一步下降到约 `42.58 kB`；
  - 预览层懒加载接入后，产物中已新增独立 chunk `AnalyticsDashboardPreview-*.js`；同时 `AnalyticsView-*.js` 主 chunk 又进一步下降到约 `39.24 kB`；
  - 继续将 [frontend/src/views/settings/WorkflowEditorModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/WorkflowEditorModal.tsx) 从 [frontend/src/views/SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 中拆出并懒加载后，产物中已新增独立 chunk `WorkflowEditorModal-*.js`；同时 `SettingsView-*.js` 主 chunk 已从约 `92.24 kB` 下降到约 `88.08 kB`；
  - 继续将 [frontend/src/views/settings/TeamMemberModals.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/TeamMemberModals.tsx) 从 [frontend/src/views/SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 中拆出并懒加载后，产物中已新增独立 chunk `TeamMemberModals-*.js`；同时 `SettingsView-*.js` 主 chunk 又进一步下降到约 `85.50 kB`；
  - 本轮继续将权限中心的两块重型矩阵面板从 [frontend/src/views/SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 中拆出为 [frontend/src/views/settings/MenuPermissionsPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/MenuPermissionsPanel.tsx) 与 [frontend/src/views/settings/ActionPermissionsPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/ActionPermissionsPanel.tsx)，并通过 `React.lazy + Suspense` 仅在切入对应 Tab 时按需加载；构建产物中已新增独立 chunk `MenuPermissionsPanel-*.js`、`ActionPermissionsPanel-*.js`，同时 `SettingsView-*.js` 主 chunk 继续从约 `85.50 kB` 下降到约 `68.11 kB`；
  - 本轮继续将 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 中仅在点击审批时才会使用的重型审批工作台模态层拆出为 [frontend/src/views/opportunities/OpportunityApprovalModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/OpportunityApprovalModal.tsx)，并通过 `React.lazy + Suspense` 按需加载；构建产物中已新增独立 chunk `OpportunityApprovalModal-*.js`，同时 `OpportunitiesDemoView-*.js` 主 chunk 从约 `54.66 kB` 下降到约 `46.70 kB`；
  - 本轮继续将 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 中仅在点击“查看”时才会使用的商机详情弹层拆出为 [frontend/src/views/opportunities/OpportunityDetailModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/OpportunityDetailModal.tsx)，并通过 `React.lazy + Suspense` 按需加载；构建产物中已新增独立 chunk `OpportunityDetailModal-*.js`，同时 `OpportunitiesDemoView-*.js` 主 chunk 从约 `46.70 kB` 继续下降到约 `42.37 kB`；
  - 本轮继续将 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 中仅在交互时才会触发的“新建/编辑商机”表单弹层和“统计概览/文档预览”辅助弹层分别拆出为 [frontend/src/views/opportunities/OpportunityEditorModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/OpportunityEditorModal.tsx) 与 [frontend/src/views/opportunities/OpportunitySupportModals.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/OpportunitySupportModals.tsx)，并通过 `React.lazy + Suspense` 按需加载；构建产物中已新增独立 chunk `OpportunityEditorModal-*.js`、`OpportunitySupportModals-*.js`，同时 `OpportunitiesDemoView-*.js` 主 chunk 从约 `42.37 kB` 继续下降到约 `39.57 kB`；
  - 本轮继续将 [frontend/src/views/ProjectsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/ProjectsView.tsx) 中体量最大的“项目详情 + 关联商机 + 阶段导航时间线”模态层拆出为 [frontend/src/views/projects/ProjectDetailModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/projects/ProjectDetailModal.tsx)，并通过 `React.lazy + Suspense` 按需加载；构建产物中已新增独立 chunk `ProjectDetailModal-*.js`，同时 `ProjectsView-*.js` 主 chunk 从约 `31.08 kB` 下降到约 `25.51 kB`；
  - 本轮继续将 [frontend/src/views/ProjectsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/ProjectsView.tsx) 中剩余的“新建/编辑项目”“预算分布”“新建关联商机”三个辅助弹层收敛为 [frontend/src/views/projects/ProjectSupportModals.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/projects/ProjectSupportModals.tsx)，并通过 `React.lazy + Suspense` 按需加载；构建产物中已新增独立 chunk `ProjectSupportModals-*.js`，同时 `ProjectsView-*.js` 主 chunk 从约 `25.51 kB` 继续下降到约 `22.35 kB`；
  - 本轮继续将 [frontend/src/views/SolutionsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SolutionsView.tsx) 中仅在交互时才会触发的“新建方案”“查看方案详情”两块辅助弹层收敛为 [frontend/src/views/solutions/SolutionSupportModals.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/solutions/SolutionSupportModals.tsx)，并通过 `React.lazy + Suspense` 按需加载；构建产物中已新增独立 chunk `SolutionSupportModals-*.js`，同时 `SolutionsView-*.js` 主 chunk 从约 `26.71 kB` 下降到约 `18.82 kB`；
  - 本轮继续将 [frontend/src/views/SolutionsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SolutionsView.tsx) 中体量最大的“方案审批流程”工作台模态层拆出为 [frontend/src/views/solutions/SolutionApprovalModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/solutions/SolutionApprovalModal.tsx)，并通过 `React.lazy + Suspense` 按需加载；构建产物中已新增独立 chunk `SolutionApprovalModal-*.js`，且本轮最终构建未引入新的循环 chunk 警告；
  - 本轮继续将 [frontend/src/views/FeishuIntegrationView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/FeishuIntegrationView.tsx) 中右侧 `MVP 预览台` 的三段重型内容按 `bindings / commands / cards` 模式拆出为 [frontend/src/views/feishu/FeishuBindingsPreviewPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/feishu/FeishuBindingsPreviewPanel.tsx)、[frontend/src/views/feishu/FeishuCommandsPreviewPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/feishu/FeishuCommandsPreviewPanel.tsx)、[frontend/src/views/feishu/FeishuCardsPreviewPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/feishu/FeishuCardsPreviewPanel.tsx)，并通过 `React.lazy + Suspense` 仅在切换到对应分段时按需加载；构建产物中已新增独立 chunk `FeishuBindingsPreviewPanel-*.js`、`FeishuCommandsPreviewPanel-*.js`、`FeishuCardsPreviewPanel-*.js`，同时 `FeishuIntegrationView-*.js` 主 chunk 从约 `24.78 kB` 下降到约 `19.54 kB`；
  - 当前仍有 `vendor-echarts-core / vendor-echarts-gl / vendor-antd` 超过 Vite 默认 `500 kB` 提示阈值，但本轮最终保留版本未引入新的构建错误或循环 chunk 警告。
- 当前结论：
  - “把 ECharts 入口从整包导入改为明确模块导入 + 继续按图表类型异步 install”已经验证有效，而且在不新增循环 chunk 警告的前提下继续拿到了体积收益；
  - 当前 `AnalyticsView` 的设计器编辑态和全屏大屏预览层都已完成独立懒加载；`SettingsView` 中工作流编辑弹窗、团队成员编辑/查看弹窗，以及权限中心菜单/动作两块矩阵面板也已拆出；`OpportunitiesDemoView` 的审批工作台、详情、新建/编辑以及统计/预览弹层也都已独立成异步 chunk；`ProjectsView` 的项目详情主模态层以及剩余辅助弹层也已拆出；`SolutionsView` 的新建、详情与审批三块模态层也已完成异步拆分；`FeishuIntegrationView` 的三段预览台也已改为按分段异步装载。下一步若继续性能线，更值得优先评估的是继续压缩 `vendor-antd / vendor-misc`，或者再检查 `OpenClawPlaygroundView` 等仍在 `20 kB+` 的页面是否还有类似的非首屏边界可拆。

### 阶段汇总（2026-04-07）

- 当前前端性能线已完成的稳定收口范围：
  - 页面级懒加载已完成，系统设置下的飞书集成、OpenClaw 联调、插件库也已完成二级按需加载；
  - `AnalyticsView / SettingsView / OpportunitiesDemoView / ProjectsView / SolutionsView / FeishuIntegrationView` 均已补齐一轮或多轮内部异步边界清理，优先拆出了设计器、预览层、审批工作台、详情模态、编辑表单、权限矩阵和分段预览台等非首屏内容；
  - 当前所有这类拆分都已通过本地 `frontend npm run build` 验证，且最终保留版本未引入新的循环 chunk 警告。
- 当前已确认的页面主 chunk 基线：
  - `AnalyticsView` 约 `39.24 kB`；
  - `SettingsView` 约 `68.11 kB`；
  - `OpportunitiesDemoView` 约 `39.57 kB`；
  - `ProjectsView` 约 `22.35 kB`；
  - `SolutionsView` 约 `18.82 kB`；
  - `FeishuIntegrationView` 约 `19.54 kB`。
- 当前剩余的主要性能热点：
  - `vendor-echarts-core` 约 `647.22 kB`；
  - `vendor-echarts-gl` 约 `599.91 kB`；
  - `vendor-antd` 约 `556.58 kB`；
  - `vendor-misc` 约 `411.00 kB`。
- 文档同步状态：
- 本轮本地性能优化进展、构建结果、chunk 变化和下一步建议已同步更新到本 `README.md`；
- 已继续补上一处飞书卡片预览交互缺陷修复：[frontend/src/views/FeishuIntegrationView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/FeishuIntegrationView.tsx) 中“从后端刷新当前示例”不再复用“命令口径”的成功提示和内部状态，而是改为按卡片类型分别请求并给出准确反馈；当前“待审批卡片”在后端返回空列表时会明确提示“暂无待审批事项，保留现有卡片示例”，避免再出现“已从后端刷新命令示例：待我审批”的串文案；
- 若下一步准备进入云端发布，应继续遵循“先本地验证，再最小范围同步云端，再做公网验收”的顺序，不应跳过本地构建验证。
- 云端同步建议：
  - 可以同步，但建议仅同步本轮涉及的前端文件与文档，不触碰 `backend/`、数据库文件或与本轮无关的部署物；
  - 建议最小同步范围至少包括 [frontend/src/views/SolutionsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SolutionsView.tsx)、[frontend/src/views/solutions/SolutionSupportModals.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/solutions/SolutionSupportModals.tsx)、[frontend/src/views/solutions/SolutionApprovalModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/solutions/SolutionApprovalModal.tsx)、[frontend/src/views/FeishuIntegrationView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/FeishuIntegrationView.tsx)、[frontend/src/views/feishu/FeishuBindingsPreviewPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/feishu/FeishuBindingsPreviewPanel.tsx)、[frontend/src/views/feishu/FeishuCommandsPreviewPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/feishu/FeishuCommandsPreviewPanel.tsx)、[frontend/src/views/feishu/FeishuCardsPreviewPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/feishu/FeishuCardsPreviewPanel.tsx) 与 [README.md](/Users/gjy/Projects-mygetpre/presales-platform/README.md)；
  - 云端动作应继续采用“上传必要文件 -> `docker compose build frontend` -> `docker compose up -d frontend` -> 公网页面复测”的非破坏式流程；
  - 在未得到用户进一步确认前，本阶段不直接执行云端同步和容器重建。

### 自动化验证补记（2026-04-07）

- 本轮已完成自动化基线验证：
  - 执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/backend && npm test`，共 15 条后端自动化用例全部通过；
  - 当前通过项覆盖飞书事件 challenge / token 校验、飞书审批卡片 stale / forbidden / duplicate / open_detail 分支、OpenClaw 只读技能映射与写意图拦截，以及密码哈希校验；
  - 执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，前端生产构建通过；
  - 在继续补上飞书卡片刷新逻辑与 token 兜底后，再次执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build` 仍然通过；
  - 最新构建结果维持本轮性能收口后的产物基线：`SolutionsView` 约 `18.82 kB`、`FeishuIntegrationView` 约 `22.29 kB`、`ProjectsView` 约 `22.35 kB`、`AnalyticsView` 约 `39.24 kB`、`SettingsView` 约 `68.11 kB`。
- 本轮已补充本地浏览器自动化冒烟验证：
  - 已在本地运行态 `http://127.0.0.1:4173/` 使用 `admin_demo / Admin@123` 成功登录平台；
  - 登录后已确认左侧主导航正常展示“工作台 / 项目管理 / 商机管理 / 解决方案 / 投标管理 / 合同管理 / 知识库 / 数据分析 / 系统设置”；
  - 已进入“系统设置”页面，确认设置入口中可见“飞书集成”与“OpenClaw联调”；
  - 已进入“飞书集成”页面，确认“绑定管理（Mock / API）”与右侧 `MVP 预览台` 正常渲染，说明本轮按 `bindings / commands / cards` 拆分后的异步面板可正常加载；
  - 已进入“解决方案”页面，确认搜索栏、统计卡、方案列表及“查看 / 下载 / 编辑 / 审批”操作入口正常渲染，说明本轮方案页模态拆分后主页面未出现渲染回归。
- 本轮已继续补充半流程自动化验证：
  - 已在“系统设置 -> OpenClaw联调”页面直接发送默认查询“给我今日简报”，成功命中 `get_daily_brief`，页面中的“本轮请求=1、成功返回=1、异常失败=0”与“最近状态”均已联动更新，说明 OpenClaw 联调页的请求发送、结果摘要和状态联动链路正常；
  - 同一次验证中，结果区已展示 `requestId`、技能名 `get_daily_brief`、命中原因 `matched_daily_brief_keywords` 以及结构化摘要字段，说明当前联调页不仅能渲染入口，还能完成一次真实只读查询回包；
  - 已继续在“系统设置 -> OpenClaw联调”页面点击“只读拦截”快捷命令并发送 `帮我审批通过 OPP-000001`，页面已明确返回“已安全拦截，未执行写操作”，同时“只读拦截=1、成功返回=0、异常失败=0”联动更新，并展示 `OPENCLAW_READONLY_ONLY` 提示，说明 OpenClaw 当前只读保护边界仍然有效，未发生误执行写操作；
  - 已继续在“系统设置 -> 飞书集成”页面切换 `MVP 预览台` 到“命令口径”，并实际运行“今日简报”示例；页面已出现“最近请求信息”和“今日简报接口返回”结果卡，同时展示目标接口、请求时间、结果 `success` 与结构化摘要字段，说明飞书集成页中“命令口径 -> 运行示例 -> 展示后端返回结构化结果”的链路正常；
  - 已继续在“系统设置 -> 飞书集成 -> 卡片预览”页面点击“从后端刷新当前示例”验证待审批卡片刷新链路；当前在后端 `GET /integrations/feishu/me/pending-approvals` 返回空列表时，页面会明确提示“后端已返回成功，但当前账号暂无待审批事项，保留现有卡片示例”，同时卡片标题、字段映射视图和动作载荷预览保持稳定，说明本轮修复后的“空结果提示准确性 + 现有预览兜底”链路已生效；
  - 已在重新登录后的“系统设置 -> 飞书集成 -> 卡片预览 -> 今日简报”链路上再次点击“从后端刷新当前示例”，浏览器实际发出 `GET /integrations/feishu/me/daily-brief` 并返回 `200`；页面中的卡片标题已从“今日简报：王经理”更新为“今日简报：示例管理员”，副标题更新为实时刷新时间，统计字段同步为 `待审批总数=0 / 高风险商机=4 / 今日更新方案=10`，摘要区也同步替换为后端最新返回内容，说明本轮卡片刷新逻辑与 token 兜底链路已在成功路径下通过验证；
  - 已继续在“系统设置 -> 飞书集成 -> 卡片预览 -> 商机摘要”链路上点击“从后端刷新当前示例”，浏览器实际发出 `GET /integrations/feishu/opportunities/OPP-000003/summary` 并返回 `200`；页面中的结构化字段已更新为后端返回的 `客户=示例制造企业 / 阶段=proposal`，摘要区同步收敛为风险摘要“成交概率低于 60%，建议复核客户预算与推进节奏。”，说明除 `daily brief` 外，商机摘要卡片也已完成真实后端刷新验证；
  - 已在“解决方案”页面点击首行“审批”，成功打开“方案审批流程”模态层；当前弹层可正常展示“当前方案审批节点 / 流程来源 / 审批记录 / 添加审批意见”等区块，并因当前账号不是该节点责任人而正确显示“当前仅允许当前待处理节点责任人执行审批动作”的只读约束，说明本轮拆出的 `SolutionApprovalModal` 已正常加载且权限约束未回归。
- 当前仍未做的自动化验证边界：
  - 尚未继续覆盖更深层的写操作链路，例如真实新增方案、审批提交、飞书集成表单提交与 OpenClaw 联调发送请求；
  - 本轮中途出现过一次 `401`，已确认根因是浏览器在强制刷新后实际回到了未登录态，`localStorage.accessToken` 为空；在重新登录后同一路径已复测通过，因此该问题当前不再视为飞书卡片刷新逻辑阻塞项，但后续自动化脚本若要跨刷新复用页面，应显式补上“刷新后检查登录态 / 必要时重新登录”的步骤；
  - 若下一步继续做平台功能验收，更适合在保持当前本地运行态的前提下，补一轮围绕“管理员登录 -> 系统设置 -> 飞书集成 / OpenClaw 联调 / 解决方案审批入口”的半流程自动化检查。

### 继续验证补记（2026-04-08）

- 本次会话按上一轮停点继续接手 `docs/feishu-openclaw-dev-plan` 分支上的未提交前端性能优化改动，先完成“读 README -> 核对当前分支与工作树 -> 重新执行本地构建”的收口动作，避免任务记录与实际代码状态再次偏离。
- 已重新执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建继续通过；当前仍仅保留 Vite 默认的 chunk size 告警，没有新增 TypeScript 报错、打包失败或循环 chunk 警告。
- 本次会话继续对 [frontend/vite.config.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/vite.config.ts) 做了一轮更细粒度的稳定分桶：
  - 已不再把所有 `echarts` 强制塞进单一 `vendor-echarts-core`，而是改为“运行时 + 图表能力块”模式，新增 `vendor-echarts-bar / line / pie / scatter / effect-scatter / graph / gauge / sankey / geo` 等能力 chunk，保留 `vendor-echarts-runtime` 作为共享运行时；
  - 已将 `@ant-design/icons` 从 `vendor-antd` 中独立拆出为 `vendor-antd-icons`，降低 antd 主 vendor 的集中度；
  - 中途尝试过继续拆 `@ant-design/cssinjs / colors / fast-color` 为 `vendor-antd-styles`，但会引入 `vendor-antd-styles -> vendor-misc -> vendor-antd-styles` 循环 chunk；该方案已明确回退，不作为当前稳定基线的一部分。
  - 本次又继续尝试过把 `antd` 的 `table / form / modal / select / upload` 按子路径拆成独立 vendor；虽然构建产物一度把 `vendor-antd` 压到约 `373.58 kB`，但会同时引入 `vendor-antd-form -> vendor-antd -> vendor-antd-form`、`vendor-antd -> vendor-antd-select -> vendor-antd`、`vendor-antd -> vendor-antd-modal -> vendor-antd` 等循环 chunk，因此该方案同样已回退，不作为后续继续优化的推荐方向。
  - 在确认 `antd` 子路径分桶不稳后，本次继续把 [frontend/src/App.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/App.tsx) 中仍然静态引入的 [frontend/src/views/LoginView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/LoginView.tsx) 与 [frontend/src/views/WorkbenchView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/WorkbenchView.tsx) 改为 `React.lazy + Suspense` 按需加载，并分别补充了登录页与工作台的加载占位；
  - 这一步没有新增循环 chunk 或构建错误，且已在产物中稳定新增独立 chunk `LoginView-*.js`（约 `6.21 kB`）与 `WorkbenchView-*.js`（约 `14.17 kB`）。
  - 本次继续沿“应用壳层减重”往下清理 [frontend/src/App.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/App.tsx) 中的静态共享依赖：已去掉对 [frontend/src/shared/opportunityDemoData.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/shared/opportunityDemoData.ts) 与 [frontend/src/shared/pipelineMock.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/shared/pipelineMock.ts) 的首屏静态导入，改为“登录后异步加载共享商机示例数据 + 本地事件订阅更新”，同时将顶栏通知所需的少量判断改回 `App.tsx` 内就地计算；
  - 这一步已在构建产物中稳定把共享示例逻辑拆为独立 chunk，例如 `opportunityDemoData-*.js`（约 `15.25 kB`）、`pipelineMock-*.js`（约 `2.79 kB`）、`teamDirectory-*.js`（约 `2.49 kB`），不再跟随应用壳层首页主入口一起同步加载。
  - 本次继续将 [frontend/src/App.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/App.tsx) 中体量最大的“已登录应用壳层”整体抽出为 [frontend/src/components/AppAuthenticatedShell.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/components/AppAuthenticatedShell.tsx)，并改为真正的 `React.lazy + Suspense` 懒加载；当前 `App.tsx` 只保留认证恢复、品牌配置、主题切换和未登录入口，登录后的菜单/顶栏/页面路由分发全部下沉到独立壳层组件；
  - 该壳层拆分在最终稳定版本中已新增独立 chunk `AppAuthenticatedShell-*.js`（约 `15.12 kB`），并确认品牌配置回调 `onChangeAppName / onChangeLogo` 与设置页联动仍然保留，没有因抽壳导致系统设置行为回归。
- 本次继续验证后的最新构建基线如下：
  - `AnalyticsView-*.js` 约 `30.91 kB`；
  - `AnalyticsCustomDashboardLayout-*.js` 约 `1.21 kB`；
  - `designerChartRuntime-*.js` 约 `7.48 kB`；
  - `AnalyticsDashboardPreview-*.js` 约 `4.90 kB`；
  - `AnalyticsDashboardDesigner-*.js` 约 `7.79 kB`；
  - `dashboardPresets-*.js` 约 `2.36 kB`；
  - `SettingsView-*.js` 约 `62.75 kB`；
  - `SettingsExtendedPanels-*.js` 约 `8.15 kB`；
  - `OpportunitiesDemoView-*.js` 约 `38.97 kB`；
  - `opportunityDetailFlowSummary-*.js` 约 `4.49 kB`；
  - `ProjectsView-*.js` 约 `22.69 kB`；
  - `SolutionsView-*.js` 约 `19.17 kB`；
  - `FeishuIntegrationView-*.js` 约 `22.42 kB`；
  - `LoginView-*.js` 约 `6.21 kB`；
  - `WorkbenchView-*.js` 约 `14.29 kB`；
  - `AppAuthenticatedShell-*.js` 约 `15.12 kB`；
  - `OpenClawAsyncPanels-*.js` 约 `6.17 kB`；
  - `opportunityDemoData-*.js` 约 `15.25 kB`；
  - `pipelineMock-*.js` 约 `2.79 kB`；
  - `index-*.js` 主入口约 `7.12 kB`；
  - `vendor-antd-icons-*.js` 约 `42.09 kB`；
  - `vendor-echarts-runtime-*.js` 约 `555.85 kB`；
  - `vendor-echarts-gl-*.js` 约 `599.96 kB`；
  - `vendor-antd-*.js` 约 `514.95 kB`；
  - `vendor-misc-*.js` 约 `411.00 kB`；
  - 各图表能力 chunk 已拆为 `vendor-echarts-bar`（约 `14.49 kB`）、`vendor-echarts-line`（约 `19.98 kB`）、`vendor-echarts-pie`（约 `12.71 kB`）、`vendor-echarts-graph`（约 `17.94 kB`）等独立产物。
- 继续核对后确认：当前最值得继续投入的热点仍然不是页面主 chunk，而是公共依赖块，优先级已调整为 `vendor-antd -> vendor-echarts-runtime -> vendor-echarts-gl`；其中 `echarts` 主体已从 2026-04-06 停点时的单块约 `1023 kB` 收敛到“`555.85 kB` 运行时 + 多个十几 KB 的能力块”结构，说明前面做的按图表类型动态注册已经真正转化成了构建产物级收益。
- 额外结论补充：以 `antd` 组件子路径为单位继续做 `manualChunks`，当前在该项目里并不稳妥；即使体积数字短期更好看，也会引入明确的循环 chunk 警告。因此，后续若继续压 `vendor-antd`，更适合优先从“减少首屏静态引用的组件范围”入手，而不是继续按 `antd/es/*` 粗暴分桶。
- 本次新增结论：继续下沉 `App.tsx` 中的静态首屏视图是有效方向。将 `LoginView` 与 `WorkbenchView` 懒加载后，主入口 `index-*.js` 已从上一轮约 `62.75 kB` 进一步下降到约 `40.58 kB`，说明当前相比继续强拆 vendor，优先清理应用壳层中的静态视图引用更稳、更容易持续拿到收益。
- 本次进一步新增结论：继续清理 `App.tsx` 中仅为顶部通知或登录后体验服务的共享示例数据，也是有效方向。将 `opportunityDemoData / pipelineMock` 从应用壳层静态依赖改为登录后异步加载后，主入口 `index-*.js` 已再次从约 `40.58 kB` 下降到约 `20.96 kB`；这说明当前主入口继续减重的最佳策略，是优先把“登录后才真正需要的数据与派生逻辑”从壳层抽离出去，而不是继续强拆公共 vendor。
- 本次最新结论：只把壳层代码“抽成单独文件”还不够，必须连入口引用一起改成真正的懒加载，收益才会落到构建产物上。本轮将 authenticated shell 改为真正的 `React.lazy` 后，主入口 `index-*.js` 已进一步从约 `20.96 kB` 下降到约 `7.12 kB`；当前应用入口已经基本收敛到“主题/鉴权/未登录页”最小职责集，后续若继续性能线，更适合把注意力重新放回登录后页面内部的重型局部边界，而不是继续反复雕刻 `App.tsx`。
- 本次继续补充的页面内优化：已将 [frontend/src/views/OpenClawPlaygroundView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpenClawPlaygroundView.tsx) 中非首屏关键的“右侧辅助侧栏 + 底部请求记录历史”拆出为 [frontend/src/views/openclaw-playground/OpenClawAsyncPanels.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/openclaw-playground/OpenClawAsyncPanels.tsx)，并通过 `React.lazy + Suspense` 按需加载；
- 本次新增页面内结论：对登录后页面继续做“主交互区同步、辅助面板异步”的细拆，同样能稳定拿到收益。当前 `OpenClawPlaygroundView-*.js` 已从约 `28.96 kB` 下降到约 `24.51 kB`，同时新增 `OpenClawAsyncPanels-*.js`（约 `6.17 kB`）；这说明接下来若继续压登录后页面，优先拆出历史记录、调试区、预览侧栏、详情抽屉这类“非首屏必需”的局部面板，会比继续动公共 vendor 更稳妥。
- 本次继续沿同一策略收口 [frontend/src/views/SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx)：已将“平台品牌与 Logo 设置”和“知识库目录管理”两块非主路径扩展面板抽出到 [frontend/src/views/settings/SettingsExtendedPanels.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/SettingsExtendedPanels.tsx)，改为 `React.lazy + Suspense` 按需加载，保留设置页主导航、团队管理、权限中心、审批流程等主交互区同步渲染。
- 本次新增设置页结论：对 `SettingsView` 继续执行“主交互区同步、辅助面板异步”的局部拆分后，`SettingsView-*.js` 已从约 `68.29 kB` 下降到约 `62.75 kB`，同时新增 `SettingsExtendedPanels-*.js`（约 `8.15 kB`）；这说明系统设置页后续若继续优化，仍应优先拆出品牌配置、目录维护、说明性面板、低频编辑器这类非首屏必需区块，而不是回到高风险的公共 vendor 强拆。
- 本次继续沿同一策略收口 [frontend/src/views/AnalyticsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/AnalyticsView.tsx)：已将“自定义仪表盘布局渲染 + 设计器图表运行时”从主页面中拆出，新增 [frontend/src/views/analytics/AnalyticsCustomDashboardLayout.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/analytics/AnalyticsCustomDashboardLayout.tsx) 与 [frontend/src/views/analytics/designerChartRuntime.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/analytics/designerChartRuntime.ts)，并让 [frontend/src/views/analytics/AnalyticsDashboardDesigner.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/analytics/AnalyticsDashboardDesigner.tsx) 与 [frontend/src/views/analytics/AnalyticsDashboardPreview.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/analytics/AnalyticsDashboardPreview.tsx) 直接复用这套异步运行时。
- 本次新增分析页结论：对 `AnalyticsView` 继续执行“默认分析看板同步、自定义布局与设计器运行时异步”的拆分后，`AnalyticsView-*.js` 已从约 `39.81 kB` 下降到约 `33.57 kB`，同时新增 `AnalyticsCustomDashboardLayout-*.js`（约 `1.21 kB`）、`designerChartRuntime-*.js`（约 `7.42 kB`）、`AnalyticsDashboardPreview-*.js`（约 `4.90 kB`）与 `AnalyticsDashboardDesigner-*.js`（约 `7.94 kB`）；这说明后续若继续压数据分析页，优先拆出自定义布局、图表运行时、预览层和编辑器这类“低频但配置重”的区块，会比继续在默认看板里堆更多运行时代码更稳妥。
- 本次继续对分析页做更细一层的收口：已把设计器专用的数据源标签表和自动摘要逻辑进一步下沉到 [frontend/src/views/analytics/designerChartRuntime.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/analytics/designerChartRuntime.ts)，让 [frontend/src/views/AnalyticsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/AnalyticsView.tsx) 只保留编辑态所需的轻量摘要草稿拼装，不再同步携带整套设计器标签常量。
- 本次新增分析页补充结论：继续压缩 `AnalyticsView` 中仅服务于设计器编辑态的配置常量后，`AnalyticsView-*.js` 又从约 `33.57 kB` 进一步下降到约 `32.68 kB`；虽然单次收益已经转小，但方向仍然成立，说明数据分析页后续若继续优化，应继续优先把“仅自定义仪表盘路径会用到的配置/标签/预览逻辑”留在异步运行时里，而不是回流到默认看板主包。
- 本次继续对分析页收口 3D 大屏预设：已将 “3D 酷炫大屏 / 3D 客户大屏” 的重型 `designerWidgets` 模板从 [frontend/src/views/AnalyticsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/AnalyticsView.tsx) 中抽出到 [frontend/src/views/analytics/dashboardPresets.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/analytics/dashboardPresets.ts)，默认仅在仪表盘列表中保留元数据；当用户从下拉切换到 3D 示例，或点击“打开 3D 酷炫大屏 / 打开 3D 客户大屏”时，再按需异步补载对应 preset。
- 本次新增分析页补充结论 2：对 `DEFAULT_DASHBOARDS` 里的 3D preset 做“元数据同步、widget 模板异步”之后，`AnalyticsView-*.js` 又从约 `32.68 kB` 进一步下降到约 `30.91 kB`，同时新增 `dashboardPresets-*.js`（约 `2.36 kB`）；这说明后续若继续优化分析页，优先抽离“只有特定大屏/高级模板才会用到的 preset 数据”依然是稳定方向。
- 本次继续补做代码层清理：已从 [frontend/src/views/AnalyticsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/AnalyticsView.tsx) 中删除此前遗留但已不再被主路径引用的旧版 `getDesignerChartBaseOption` 实现，避免分析页同时保留“已迁移到 `designerChartRuntime` 的新运行时”和“主文件里的历史死代码”两套并行模板。
- 本次补充说明：这一步主要是去除历史残留、降低后续维护噪音，因此构建基线维持不变，`AnalyticsView-*.js` 仍为约 `30.91 kB`；后续若继续压缩数据分析页，应优先寻找剩余真正同步参与默认看板渲染的状态与逻辑，而不是再反复清理已经不参与产物的死代码。
- 本次开始继续收口商机页 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx)：已将“商机详情弹层顶部流程摘要”的纯计算逻辑抽出为 [frontend/src/views/opportunities/opportunityDetailFlowSummary.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityDetailFlowSummary.ts)，并改为仅在详情弹层打开时通过动态导入计算 `detailSteps`，不再让主列表页同步装载这段详情专用摘要拼装逻辑。
- 本次新增商机页结论：对 `OpportunitiesDemoView` 执行“主列表同步、详情摘要异步”的第一步细拆后，`OpportunitiesDemoView-*.js` 已从约 `39.84 kB` 下降到约 `38.97 kB`，同时新增 `opportunityDetailFlowSummary-*.js`（约 `4.49 kB`）；这说明商机页后续若继续优化，优先拆出详情视角、审批视角、流程视角下的纯派生计算，比直接触碰列表与审批主链路更稳妥。
- 本次继续沿同一路径收口商机审批弹层：已新增 [frontend/src/views/opportunities/opportunityApprovalRuntime.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityApprovalRuntime.ts)，将审批模态与详情流程摘要共用的“状态文案、驳回索引、可操作性判定、审批对象解析、业务快照显示”等纯运行时逻辑下沉到 `opportunities/` 异步子模块中；同时 [frontend/src/views/opportunities/OpportunityApprovalModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/OpportunityApprovalModal.tsx) 改为直接消费这套运行时，不再由 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 通过 props 携带整套审批派生函数。
- 本次新增商机页结论 2：对 `OpportunitiesDemoView` 继续执行“主列表同步、审批/详情派生异步”的第二步细拆后，`OpportunitiesDemoView-*.js` 已从约 `38.97 kB` 进一步下降到约 `35.61 kB`；同时产物新增 `opportunityApprovalRuntime-*.js`（约 `7.03 kB`），`OpportunityApprovalModal-*.js` 约 `10.53 kB`，而 `opportunityDetailFlowSummary-*.js` 也因复用共享运行时从约 `4.49 kB` 下降到约 `2.55 kB`。这说明商机页后续若继续优化，最稳妥的方向依然是维持“主列表同步、详情/审批/流程派生异步”，优先把只在弹层和流程视角使用的纯计算与展示判定留在懒加载边界内，而不是回流到列表主包。
- 本次继续把审批快照记录生成收口到懒加载边界：原先由 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 同步生成的 `snapshotApprovalRecords` 已下沉到 [frontend/src/views/opportunities/OpportunityApprovalModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/OpportunityApprovalModal.tsx) 内部，并在 [frontend/src/views/opportunities/opportunityApprovalRuntime.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityApprovalRuntime.ts) 中补充了共享的 `buildSnapshotApprovalRecords` 运行时；主页面只保留“是否走业务快照视图”的轻量判定，不再同步携带快照记录拼装本身。
- 本次新增商机页结论 3：对商机审批视角继续执行“主列表只保留开关判断、快照记录在审批模态内部按需生成”后，`OpportunitiesDemoView-*.js` 又从约 `35.61 kB` 进一步下降到约 `34.14 kB`；与此同时 `OpportunityApprovalModal-*.js` 维持约 `10.53 kB`，`opportunityApprovalRuntime-*.js` 约 `8.42 kB`，`opportunityDetailFlowSummary-*.js` 仍约 `2.55 kB`。这进一步说明商机页后续应继续沿着“审批打开后才需要的派生数据全部留在懒加载边界内”推进，而不是让列表主包提前背负审批快照、审批记录映射和流程只读视图的计算成本。
- 本次继续把“真实审批实例回写商机本地状态”的同步补丁构建器从主页面拆出：已新增 [frontend/src/views/opportunities/opportunityApprovalSync.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityApprovalSync.ts)，将 `syncOpportunityFromApprovalInstance` 背后的 `workflowRecords` 映射、字段回填、阶段推进等纯 patch 计算下沉到异步子模块中；[frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 现在只在“打开审批弹层并成功拉到真实实例”或“提交审批动作成功”后按需动态导入该同步模块，不再让主列表首屏同步携带这段真实审批实例映射逻辑。
- 本次新增商机页结论 4：对商机审批链路继续执行“主列表同步、真实审批回写补丁异步”之后，`OpportunitiesDemoView-*.js` 又从约 `34.14 kB` 进一步下降到约 `31.85 kB`；新增 `opportunityApprovalSync-*.js`（约 `2.25 kB`），而 `OpportunityApprovalModal-*.js` 仍约 `10.53 kB`，`opportunityApprovalRuntime-*.js` 仍约 `8.42 kB`。这说明商机页继续优化时，除了详情/审批视图本身，连“审批打开后才会触发的本地状态回写补丁”也值得留在异步边界内，避免主列表提前携带真实审批实例到本地字段的映射成本。
- 本次继续把“本地审批/上传动作后的流程推进与下一节点通知”从主页面拆出：已新增 [frontend/src/views/opportunities/opportunityLocalWorkflow.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityLocalWorkflow.ts)，将 `appendWorkflowRecords` 背后的“下一待处理节点判定、下一节点处理人解析、系统通知记录拼装”等本地流程推进逻辑下沉为异步 runtime；[frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 现在只在本地上传需求文档、上传调研文档、分配承接售前、审批通过/驳回等动作触发时按需动态导入该模块，不再让列表主包同步背负这段本地工作流推进逻辑。
- 本次新增商机页结论 5：对商机本地审批链路继续执行“主列表同步、本地流程推进异步”后，`OpportunitiesDemoView-*.js` 又从约 `31.85 kB` 进一步下降到约 `29.49 kB`；新增 `opportunityLocalWorkflow-*.js`（约 `3.23 kB`），而 `opportunityApprovalSync-*.js` 约 `2.25 kB`、`opportunityApprovalRuntime-*.js` 约 `8.42 kB`、`OpportunityApprovalModal-*.js` 约 `10.53 kB`。这说明商机页后续若继续优化，凡是“只有审批弹层内部动作才会触发的本地流程推进、通知拼装、状态回写”都值得继续留在懒加载边界中，列表主包应尽量只保留筛选、表格展示与打开弹层所需的最小入口。
- 本次继续把商机 API 适配层从主页面拆出：已新增 [frontend/src/views/opportunities/opportunityApi.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityApi.ts)，将“后端 `ApiOpportunity` -> 前端 `DemoOpportunity` 映射”、商机列表接口加载、单条商机详情接口加载等逻辑统一收口到异步模块中；[frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 现在只在初次拉取商机列表、打开详情弹层补拉商机详情、打开审批弹层补拉商机详情时按需导入该适配模块，不再同步携带整段 API 字段映射和列表/详情请求适配逻辑。
- 本次新增商机页结论 6：对商机页继续执行“主列表同步、列表/详情 API 适配异步”之后，`OpportunitiesDemoView-*.js` 又从约 `29.49 kB` 进一步下降到约 `28.34 kB`；新增 `opportunityApi-*.js`（约 `1.88 kB`），其余 `opportunityApprovalSync-*.js`、`opportunityLocalWorkflow-*.js`、`opportunityApprovalRuntime-*.js`、`OpportunityApprovalModal-*.js` 均维持稳定。说明商机页下一阶段若继续优化，除了审批/详情纯派生逻辑，连“只在数据拉取时才会用到的字段映射与接口适配”也适合留在异步边界中，继续削减列表首屏主包负担。
- 本次继续把商机流程定义 API 适配层从主页面拆出：已新增 [frontend/src/views/opportunities/opportunityWorkflowApi.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityWorkflowApi.ts)，将“后端流程定义结构 -> 前端 `WorkflowDefinition` 归一化”、节点审批人展示字段拼装，以及商机流程定义接口加载统一收口到异步模块中；[frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 现在只在页面初始化拉取商机流程定义时按需导入这段 API 适配逻辑，不再让主包同步携带整套后端流程定义类型与归一化实现。
- 本次新增商机页结论 7：对商机页继续执行“主列表同步、流程定义 API 适配异步”之后，`OpportunitiesDemoView-*.js` 又从约 `28.34 kB` 进一步下降到约 `27.39 kB`；新增 `opportunityWorkflowApi-*.js`（约 `1.47 kB`），其余 `opportunityApi-*.js`、`opportunityApprovalSync-*.js`、`opportunityLocalWorkflow-*.js`、`opportunityApprovalRuntime-*.js` 与 `OpportunityApprovalModal-*.js` 仍保持稳定。说明商机页这一轮继续优化的稳定规律已经比较明确：凡是“仅在页面初始化拉流程定义、打开详情/审批、执行本地/真实审批动作时才会触发”的 API 适配、状态映射与运行时判定，都值得优先移出列表主包。
- 本次继续把详情弹层侧的轻交互 helper 从主页面拆出：已新增 [frontend/src/views/opportunities/opportunityDetailActions.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityDetailActions.ts)，将“详情中的阶段推进规则”和“文档下载占位实现”下沉到详情侧异步模块中；[frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 现在只在用户点击详情里的“推进阶段”或文档下载入口时按需导入这段交互 helper，不再让列表主包同步携带这类只在详情/预览路径才会运行的轻量实现。
- 本次新增商机页结论 8：对商机页继续执行“主列表同步、详情交互 helper 异步”之后，`OpportunitiesDemoView-*.js` 又从约 `27.39 kB` 进一步下降到约 `27.09 kB`；新增 `opportunityDetailActions-*.js`（约 `0.53 kB`），其余 `opportunityWorkflowApi-*.js`、`opportunityApi-*.js`、`opportunityApprovalSync-*.js`、`opportunityLocalWorkflow-*.js` 与 `opportunityApprovalRuntime-*.js` 均维持稳定。这说明商机页这一轮后段优化依然有效，但收益已经开始变小，后续若继续压缩，应该优先寻找仍残留在主包中的“只在详情/审批/预览打开后才会触发的交互实现”，而不是冒险触碰列表主路径或 `vendor` 级公共依赖。
- 本次继续把新建/编辑商机提交时的字段归一化逻辑从主页面拆出：已新增 [frontend/src/views/opportunities/opportunityEditorActions.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityEditorActions.ts)，将“编号生成、金额与加权价值格式化、项目绑定模式解析、项目名/项目 Key 归一化、新建/编辑 payload 构建”等低频表单提交流程下沉到异步 helper 中；[frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 现在只在点击编辑弹层的“保存”时按需导入该模块，不再让主包同步携带整段编辑提交归一化逻辑。
- 本次新增商机页结论 9：对商机页继续执行“主列表同步、编辑提交流程异步”之后，`OpportunitiesDemoView-*.js` 又从约 `27.09 kB` 进一步下降到约 `26.11 kB`；新增 `opportunityEditorActions-*.js`（约 `1.58 kB`），同时 `opportunityDetailActions-*.js`、`opportunityWorkflowApi-*.js`、`opportunityApi-*.js`、`opportunityApprovalSync-*.js`、`opportunityLocalWorkflow-*.js` 与 `opportunityApprovalRuntime-*.js` 维持稳定。到这一轮为止，商机页主包已从约 `38.97 kB` 收敛到约 `26.11 kB`，说明“只在详情/审批/编辑弹层或初始化接口读取时才需要的逻辑全部迁到异步 helper”这条路线在当前代码结构下仍然是最稳、收益最明确的优化方向。
- 本次继续把点击“编辑”时的表单预填充组装从主页面拆出：已新增 [frontend/src/views/opportunities/opportunityEditorPrefill.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityEditorPrefill.ts)，将“由当前商机记录推导编辑表单默认值、项目绑定模式默认值、预期金额反格式化”等只在点击编辑入口时才会执行的轻量预处理逻辑下沉到异步 helper 中；[frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 现在只在点击列表行内“编辑”按钮后按需导入这段预填充逻辑，不再让列表主包同步携带这部分低频点击路径实现。
- 本次新增商机页结论 10：对商机页继续执行“主列表同步、编辑预填充异步”之后，`OpportunitiesDemoView-*.js` 又从约 `26.11 kB` 进一步下降到约 `25.95 kB`；新增 `opportunityEditorPrefill-*.js`（约 `0.61 kB`），其余 `opportunityEditorActions-*.js`、`opportunityDetailActions-*.js`、`opportunityWorkflowApi-*.js`、`opportunityApi-*.js`、`opportunityApprovalSync-*.js`、`opportunityLocalWorkflow-*.js` 与 `opportunityApprovalRuntime-*.js` 仍维持稳定。到当前这一步，商机页主包已从约 `38.97 kB` 收敛到约 `25.95 kB`，说明这一轮继续压缩后，剩余可拆内容大多只会带来很小的边际收益，后续应更谨慎评估每一次新增切分边界是否值得。
- 本次继续把审批弹层里的本地上传/分配/审批分支判断从主页面抽出：已新增 [frontend/src/views/opportunities/opportunityApprovalLocalPlan.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityApprovalLocalPlan.ts)，将“上传需求说明 / 上传调研文档 / 分配承接售前 / 审批通过 / 审批驳回”对应的本地 patch、流程记录草稿、提示文案和是否关闭弹窗的计划生成逻辑下沉到懒加载的审批视图边界；[frontend/src/views/opportunities/OpportunityApprovalModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/OpportunityApprovalModal.tsx) 现在直接在弹层内部组装本地审批计划，再由 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 统一应用计划并回写状态，不再在列表主包中保留整段本地审批分支闭包。
- 本次新增商机页结论 11：对商机页继续执行“主列表同步、审批弹层本地动作计划异步”之后，`OpportunitiesDemoView-*.js` 又从约 `25.95 kB` 进一步下降到约 `24.68 kB`；同时 `OpportunityApprovalModal-*.js` 上升到约 `11.62 kB`，新增 `opportunityApprovalLocalPlan-*.js` 的逻辑已并入审批弹层懒加载边界，`opportunityApprovalRuntime-*.js` 仍约 `8.42 kB`、`opportunityApprovalSync-*.js` 约 `2.25 kB`、`opportunityLocalWorkflow-*.js` 约 `3.23 kB`。这说明当前对商机页继续减重的稳定策略依然成立：凡是只有在审批弹层打开后才会用到的本地动作分支、提示文案和 patch 组装，都应尽量留在懒加载边界内，而不是让列表主包提前承担这些低频逻辑。
- 本次继续把“本地审批动作生成流程记录”的实现从主页面挪进异步工作流运行时：已将 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 中仅服务于本地审批链路的 `buildWorkflowRecord` / `getCurrentOperatorLabel` 收口到 [frontend/src/views/opportunities/opportunityLocalWorkflow.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityLocalWorkflow.ts) 内部，由该异步模块在执行 `appendWorkflowRecords` 时直接基于本地审批计划草稿、当前操作者和团队目录生成正式 `workflowRecords`；主页面现在只负责传递 `recordDrafts`、当前登录用户名和标签函数，不再同步携带整段本地流程记录组装实现。
- 本次新增商机页结论 12：对商机页继续执行“主列表同步、流程记录构造异步”之后，`OpportunitiesDemoView-*.js` 又从约 `24.68 kB` 进一步下降到约 `24.31 kB`；同时 `opportunityLocalWorkflow-*.js` 从约 `3.23 kB` 上升到约 `3.76 kB`，其余 `OpportunityApprovalModal-*.js` 约 `11.62 kB`、`opportunityApprovalRuntime-*.js` 约 `8.42 kB`、`opportunityApprovalSync-*.js` 约 `2.25 kB` 保持稳定。说明在当前结构下，继续把“仅在本地审批动作触发时才会执行的流程记录拼装和操作者标签计算”留在异步工作流边界内，仍然能稳定换回列表主包减重。
- 本次继续清理商机页里重构后遗留的无引用死逻辑：已从 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 中删除不再参与当前渲染与交互的 `getWorkflowNode`、`isWorkflowNodeCompleted`、`getStageDrivenWorkflowNodeIndex`、`STAGE_COLORS`、`stageStats` 以及仅被这些死逻辑引用的 `getWorkflowNodeDocumentName`。这一步不改变当前列表、详情、审批、编辑路径行为，只是去掉此前多轮拆分后残留在主包里的无效派生代码。
- 本次新增商机页结论 13：对商机页执行“清理无引用死逻辑”之后，`OpportunitiesDemoView-*.js` 又从约 `24.31 kB` 进一步下降到约 `23.91 kB`；同时其余 `OpportunityApprovalModal-*.js` 约 `11.62 kB`、`opportunityApprovalRuntime-*.js` 约 `8.42 kB`、`opportunityApprovalSync-*.js` 约 `2.25 kB`、`opportunityLocalWorkflow-*.js` 约 `3.76 kB` 保持稳定。说明当前商机页的最后一段收益，已经开始更多来自“删除重构后未再使用的主包残留逻辑”，而不是继续新增异步边界。
- 本次开始把性能线切回业务闭环补口：已将 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 中“打开商机审批弹窗时只查询 `current approval instance`，查不到就回退前端本地链路”的逻辑，改为直接复用 [frontend/src/shared/approvalInstances.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/shared/approvalInstances.ts) 里已经存在的 `ensureApprovalInstance()`；当前商机审批入口会像方案页一样，在检测不到现有审批实例时自动调用后端 `POST /approval-instances/start` 启动真实实例，再继续走真实审批视图，而不是第一时间回退到本地演示链路。
- 本次业务闭环补充结论 1：这一步说明当前“商机审批真实闭环”的主缺口不在后端审批能力本身，而在前端商机页尚未把现有后端启动能力真正接上。补齐后，本地构建继续通过，`OpportunitiesDemoView-*.js` 仍保持稳定并从约 `23.91 kB` 微降到约 `23.87 kB`；因此后续若继续推进商机审批真实化，应优先围绕“前端何时启动实例、何时展示真实实例、何时才允许回退本地链路”的策略继续收口，而不是再重复建设后端审批基础设施。
- 本次继续收口“真实实例 vs 业务快照”的显示优先级：已在 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx)、[frontend/src/views/opportunities/opportunityApprovalRuntime.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityApprovalRuntime.ts) 与 [frontend/src/views/opportunities/opportunityDetailFlowSummary.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityDetailFlowSummary.ts) 增加“本次已通过 `ensureApprovalInstance()` 明确拿到真实实例时，不再让 `actions = 0` 的业务快照只读视图覆盖真实实例”的控制口径；这样商机审批弹层和详情侧流程摘要都会优先展示后端真实节点状态与可操作性，而不再因为历史业务字段已推进就把刚确保出来的真实审批流遮掉。
- 本次业务闭环补充结论 2：为避免“真实实例刚创建但尚无动作记录”时提前用空实例把前端历史记录清空，本轮没有在 `actions.length === 0` 时执行本地商机字段回写；只有真实审批动作落库后，才继续通过 [frontend/src/views/opportunities/opportunityApprovalSync.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/opportunityApprovalSync.ts) 同步 `workflowRecords`、审批状态与阶段字段。这样既保住了真实审批流入口可操作，又避免了旧快照数据在首次打开弹层时被误覆盖。
- 本次业务闭环补充结论 3：本地重新执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build` 后继续通过；当前构建产物中 `OpportunitiesDemoView-*.js` 约 `24.02 kB`、`OpportunityApprovalModal-*.js` 约 `11.62 kB`、`opportunityApprovalRuntime-*.js` 约 `8.48 kB`、`opportunityDetailFlowSummary-*.js` 约 `2.64 kB`。这说明这一步主要是行为校准而非继续压包，后续商机审批线更适合围绕“真实实例首次创建后的初始节点显示、详情摘要一致性、真实动作后的回写粒度”继续做业务验收。
- 浏览器回归补记（确认日期：2026-04-08）：已在本地提权启动 `backend start:dev (3000)` 与 `frontend vite dev (5173)` 后，用真实浏览器按“销售账号 -> 商机审批 -> 经理账号继续审批”完成一轮真链路复测。样本商机选用 [OPP-000001](/Users/gjy/Projects-mygetpre/presales-platform/README.md) 对应的“某银行数字化转型项目”，其初始业务快照已存在需求说明文档，但本地库中尚无任何 `businessType='opportunity'` 的审批实例，正好覆盖“首次打开审批时自动创建真实实例”的目标场景。
- 浏览器回归补记（确认日期：2026-04-08）：以 `zhangsan_sales / Sales@123` 登录后进入“商机管理 -> OPP-000001 -> 审批”，弹层已直接显示“流程来源：后端真实审批实例”，且首节点“线索确认”呈现为真实实例的“进行中”可操作态，而不再出现此前“后端实例无动作时退回业务快照只读视图”的提示。对应数据库中已自动生成 `approval_instances.id=2 / businessType=opportunity / businessId=1 / status=in_progress`，说明 `ensureApprovalInstance()` 的自动启动链路已真实打通。
- 浏览器回归补记（确认日期：2026-04-08）：继续在同一弹层上传本地文件名 `README.md` 后，数据库已写入 `approval_actions.id=1 / approvalInstanceId=2 / approvalInstanceNodeId=4 / actionType=upload`，实例当前节点从 `approval_instance_nodes.id=4 (lead_confirmation)` 流转到 `id=5 (sales_leader_approval)`；页面也同步改为“线索确认 = 已上传”“销售领导审批 = 进行中”，审批记录区出现“张三 · 线索确认 · 上传需求说明”。这说明真实动作写入、节点推进与前端展示已经对齐。
- 浏览器回归补记（确认日期：2026-04-08）：随后清空登录态并改用 `manager_demo / Manager@123` 重新进入同一审批弹层，节点 2“销售领导审批”的通过/驳回按钮已从置灰恢复为可点击，证明真实实例的“当前处理人”切换没有被前端快照逻辑拦住。进一步点击“通过”后，数据库新增 `approval_actions.id=2 / approvalInstanceNodeId=5 / actionType=approve`，实例 `currentNodeId` 已推进到 `6 (solution_leader_approval)`；页面同步显示“销售领导审批 = 已通过”“解决方案领导审批 = 进行中”，审批记录区新增“示例经理 · 销售领导审批 · 审批通过”。
- 浏览器回归补记（确认日期：2026-04-08）：本轮真链路验证结论是，当前商机审批的“首次自动建实例 -> 销售上传首节点 -> 经理接力审批下一节点”闭环已经在本地真实前后端跑通；因此上一轮关于“真实实例会被业务快照遮掉、导致刚创建实例也只能只读查看”的主缺口可以认为已完成浏览器级验收。
- 浏览器回归补记（确认日期：2026-04-08）：同时在浏览器控制台仍观察到一个残余前端问题：进入 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 时会重复出现 React `Maximum update depth exceeded` 与“Cannot update a component while rendering a different component”警告，说明商机页还存在一个循环更新点。该问题未阻塞本轮真实审批闭环验收，但应作为下一步前端稳定性排查的优先项，避免后续继续在真链路验证时放大页面抖动与控制台噪音。
- 前端稳定性补记（确认日期：2026-04-09）：已继续排查上轮浏览器回归中暴露的 `OpportunitiesDemoView` 循环更新问题，根因确认不是审批运行时本身，而是共享商机缓存的“自写自回放”回路：商机页在 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 的 `useEffect` 中会对每次 `opportunities` 变更执行 `saveSharedDemoOpportunities(opportunities)`；而 [frontend/src/shared/opportunityDemoData.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/shared/opportunityDemoData.ts) 中的 `saveSharedDemoOpportunities()` 又会无条件 `dispatchEvent(OPPORTUNITY_DEMO_UPDATED_EVENT)`，导致商机页自己监听到自己刚广播的数据并再次 `setOpportunities(customEvent.detail)`，从而形成“保存 -> 广播 -> 自己写回自己”的重复更新链。
- 前端稳定性补记（确认日期：2026-04-09）：当前已在 [frontend/src/shared/opportunityDemoData.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/shared/opportunityDemoData.ts) 为 `saveSharedDemoOpportunities()` 增加幂等保护：会先比较即将写入的规范化 JSON 与当前 localStorage 中已保存的商机快照及版本号，只有在内容或版本实际发生变化时才执行 `setItem` 和 `dispatchEvent`。这样工作台等依赖共享商机事件的页面仍能收到真实变更，但商机页不会再因为“保存的内容与当前缓存完全相同”而被自己再次打回。
- 前端稳定性补记（确认日期：2026-04-09）：本次修复后已重新执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建继续通过；当前产物中 `OpportunitiesDemoView-*.js` 仍约 `24.02 kB`、`opportunityDemoData-*.js` 约 `15.35 kB`，说明这一步属于稳定性修复，没有引入额外体积回退。浏览器层面的二次复测本轮未完成收尾，因为 Playwright 会话在重启本地 dev server 后已失效；但基于根因与补丁口径，下一次继续时应优先重新开干净浏览器会话，确认“进入商机页后控制台不再刷 `Maximum update depth exceeded`，且审批弹层仍可正常打开”。
- 前端稳定性补记（确认日期：2026-04-09）：为进一步降低未来同类问题复发概率，本次又继续在共享商机事件的接收侧补了第二层幂等保护： [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 与 [frontend/src/views/WorkbenchView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/WorkbenchView.tsx) 现在在接收 `sharedDemoOpportunitiesUpdated` 事件时，会先通过 [frontend/src/shared/opportunityDemoData.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/shared/opportunityDemoData.ts) 中新增的 `hasSameDemoOpportunities()` 比较当前状态与广播内容；若内容完全相同，则直接复用旧 state，不再触发无意义的 `setState`。这意味着即便未来仍有别的模块重复广播相同的共享商机快照，商机页与工作台也不会再因为“同内容不同引用”的事件 payload 产生额外重渲染。
- 前端稳定性补记（确认日期：2026-04-09）：在补齐接收侧幂等后，再次执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build` 仍然通过；最新产物中 `OpportunitiesDemoView-*.js` 约 `24.05 kB`、`WorkbenchView-*.js` 约 `14.31 kB`、`opportunityDemoData-*.js` 约 `15.45 kB`。这一步依然属于稳定性加固，不改变业务链路与审批闭环，只是把“共享商机广播 -> 页面消费”的数据同步边界收紧到“真正有变更时才更新”。
- 前端稳定性补记（确认日期：2026-04-09）：在继续做浏览器回归时，又定位到剩余那条 `Cannot update a component while rendering a different component` 告警并不来自“保存后广播”本身，而是来自 [frontend/src/shared/opportunityDemoData.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/shared/opportunityDemoData.ts) 的 `loadSharedDemoOpportunities()` 在“读取共享商机数据”时仍可能因为版本升级或默认示例补齐而直接调用 `saveSharedDemoOpportunities()`。这意味着某些页面在初始化 `useState(() => loadSharedDemoOpportunities())` 的渲染阶段就会触发全局广播，进而命中 React 对“渲染期间更新其他组件”的告警。
- 前端稳定性补记（确认日期：2026-04-09）：当前已把这条副作用从 render 路径中剥离： [frontend/src/shared/opportunityDemoData.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/shared/opportunityDemoData.ts) 中的 `loadSharedDemoOpportunities()` 现在只负责纯读取与归一化，不再在读取时写 localStorage 或派发事件；同时新增 `ensureSharedDemoOpportunities()`，专门用于在 effect-safe 场景下做“必要时补齐默认数据并持久化”的同步动作。此外，[frontend/src/components/AppAuthenticatedShell.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/components/AppAuthenticatedShell.tsx) 初始化共享商机状态时已切换为调用 `ensureSharedDemoOpportunities()`，并在事件监听侧补上 `hasSameDemoOpportunities()` 守卫，避免壳层因为重复 payload 产生无意义的状态写回。
- 浏览器回归补记（确认日期：2026-04-09）：本次已使用本机 Chrome 执行最小真实回归：访问 `http://127.0.0.1:5173`，使用 `zhangsan_sales / Sales@123` 登录，进入“商机管理”，再打开 `OPP-000001` 的审批弹层。结果确认如下：登录成功、商机页可正常渲染、审批弹层可正常打开并展示“流程来源：后端真实审批实例”；控制台中此前反复出现的 React `Cannot update a component while rendering a different component` 告警本轮已不再出现，也未再观测到 `Maximum update depth exceeded`。当前控制台只剩两个既有的 Ant Design 弃用提示：`Card bodyStyle` 与 `Modal destroyOnClose`。
- 前端稳定性补记（确认日期：2026-04-09）：在完成上述修复后，再次执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build` 仍然通过；最新产物中 `OpportunitiesDemoView-*.js` 约 `24.05 kB`、`WorkbenchView-*.js` 约 `14.31 kB`、`AppAuthenticatedShell-*.js` 约 `15.22 kB`、`opportunityDemoData-*.js` 约 `15.62 kB`。这说明本轮属于“把共享商机初始化恢复为纯读取，并把必要的持久化同步移到 effect-safe 边界”的稳定性修复，没有引入新的明显包体回退。
- 前端稳定性补记（确认日期：2026-04-09）：已继续沿着同一条真实回归路径清理控制台噪音：将 [frontend/src/views/WorkbenchView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/WorkbenchView.tsx) 与 [frontend/src/views/OpportunitiesDemoView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpportunitiesDemoView.tsx) 中当前实际命中的 `Card bodyStyle` 改为 `styles.body`，并将 [frontend/src/views/opportunities/OpportunityApprovalModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/OpportunityApprovalModal.tsx) 与 [frontend/src/views/opportunities/OpportunityEditorModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/opportunities/OpportunityEditorModal.tsx) 中的 `destroyOnClose` 改为 `destroyOnHidden`。这样处理后，工作台、商机页主视图和审批/编辑弹层在当前 antd 版本下都不再触发对应弃用提示。
- 浏览器回归补记（确认日期：2026-04-09）：再次使用本机 Chrome 执行“登录 -> 工作台 -> 商机管理 -> OPP-000001 审批弹层”的最小真实回归后，控制台已不再出现 `Card bodyStyle` 与 `Modal destroyOnClose` 弃用告警；当前仅剩 Vite HMR 的 `connecting / connected` 调试日志和 React DevTools 提示，不再存在 React 渲染期更新告警、最大更新深度告警或本路径下的 Ant Design 弃用噪音。至此，本轮商机页稳定性收口可以视为完成。
- 真实审批闭环补记（确认日期：2026-04-09）：在控制台告警清零后，本次继续用真实后端接口把 `approval_instances.id=2` 对应的商机审批链完整推进到了终态。执行顺序为：`manager_demo` 审批通过 `solution_leader_approval`，随后分配 `presales_demo` 为解决方案负责人，再由 `presales_demo` 上传 `某银行_需求调研纪要_v1.0.docx` 完成 `requirement_analysis`，最后由 `manager_demo` 提交“最终审批通过，商机推进条件满足”完成 `final_approval`。当前实例状态已为 `approved`，`currentNodeId=null`，6 个节点全部为 `approved`，对应动作流水现已完整覆盖 `upload -> approve -> approve -> assign -> upload -> approve`。
- 真实审批闭环补记（确认日期：2026-04-09）：以当前样本商机 `OPP-000001 / opportunities.id=1` 为例，真实业务字段现已与审批实例保持一致：`approvalStatus=approved`、`bizApprovalStatus=approved`、`techApprovalStatus=approved`、`solutionOwnerUsername=presales_demo`、`researchDocName=某银行_需求调研纪要_v1.0.docx`、`approvalOpinion=最终审批通过，商机推进条件满足`。这说明此前仅验证到“解决方案领导审批进行中”的商机真实审批链，本轮已经补齐到完整终态。
- 后端同步修复补记（确认日期：2026-04-09）：在推进完整审批链时又发现一个后端一致性缺口：`ApprovalsService.syncBusinessApprovalState()` 原先只按实例总状态回写商机 `approvalStatus`，没有基于实例节点状态补齐 `bizApprovalStatus / techApprovalStatus`；同时它通过整实体 `save()` 回写审批字段，存在误覆盖其它可空业务字段的风险。当前已在 [backend/src/approvals/approvals.service.ts](/Users/gjy/Projects-mygetpre/presales-platform/backend/src/approvals/approvals.service.ts) 修复为：
  1. 基于实例节点 `fieldKeySnapshot` 推导并回写 `bizApprovalStatus / techApprovalStatus`；
  2. 将审批状态同步从整实体 `save()` 改为定向 `update()`，避免覆盖 `solutionOwnerUsername / researchDocName / approvalOpinion` 等其它业务字段；
  3. 在读取 `findOne / findLatestForBusiness` 时保留轻量自愈，使历史上已经存在的审批实例也能在读取时自动把审批摘要字段修正到最新真实状态。
- 后端验证补记（确认日期：2026-04-09）：修复后已重新执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/backend && npm run build`，构建通过；随后再次通过本地接口与 SQLite 实体文件核对，确认当前运行中的后端进程确实使用项目根目录下的 [pre_sales_lifecycle.sqlite](/Users/gjy/Projects-mygetpre/presales-platform/pre_sales_lifecycle.sqlite)，且接口返回与数据库记录已完全一致，不再出现“审批实例已完成但商机业务摘要字段仍滞后或被误清空”的偏差。
- 浏览器终态验收补记（确认日期：2026-04-09）：在上述后端修复完成后，已再次使用本机 Chrome 以 `zhangsan_sales / Sales@123` 执行“登录 -> 商机管理 -> OPP-000001 审批弹层”终态验收。结果确认如下：商机列表中 `OPP-000001` 的审批状态已显示为“已批准”；审批弹层中 6 个节点全部表现为已处理完成，其中“解决方案领导审批”“最终审批”均为“已通过”，“需求分析”为“已上传”，“分配解决方案负责人”已展示 `示例售前（presales_demo）`；审批记录区也已展示最终审批意见“最终审批通过，商机推进条件满足”以及需求分析阶段上传的 `某银行_需求调研纪要_v1.0.docx`。这说明从真实审批实例到前端终态展示的整条链路，本轮已经完成浏览器级验收。
- 云上最小发布补记（确认日期：2026-04-09）：本轮已按“只同步最近两次提交改动文件”的口径，从本地 `/Users/gjy/Projects-mygetpre/presales-platform` 生成最小上传包并发布到云服务器 `deploy@101.43.78.27:/opt/presales-platform`。首次最小包只覆盖了最近两次提交内显式变更的 47 个文件，导致云端 `frontend` 构建时缺少 `frontend/src/views/openclaw-playground/helpers.tsx` 等同目录依赖文件，报错 `Could not resolve "./openclaw-playground/helpers"`；随后已补传整个 [frontend/src/views/openclaw-playground](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/openclaw-playground) 目录的热修复小包，并仅重建 `frontend` 容器完成收口。该问题说明后续做“最小发布”时，除 `git diff` 文件外，还需把被这些入口文件依赖但未在当前提交中改动的同目录模块一并纳入同步范围。
- 云上联调补记（确认日期：2026-04-09）：热修复后，云端重新执行 `docker compose build backend`、`docker compose build frontend` 与 `docker compose up -d frontend`，当前 `presales-mysql / presales-backend / presales-frontend` 均已恢复运行；`docker compose ps` 显示 `mysql` 为 `healthy`、`frontend` 暴露 `0.0.0.0:80->80/tcp`，`backend` 正常启动并完成 Nest 路由映射。云端进一步确认 `curl http://127.0.0.1/api/health` 与 `curl http://101.43.78.27/api/health` 均返回 `{"status":"ok"}`，同时 `curl http://101.43.78.27/` 已能返回新的前端入口 HTML，脚本入口为 `index-RrxAhjRK.js`。
- 云上功能验收补记（确认日期：2026-04-09）：由于本机 headless Chrome 在执行现有 [output/playwright/opportunity-regression-check.mjs](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/opportunity-regression-check.mjs) 时发生 `SIGABRT`，本轮未直接复用浏览器脚本跑公网 UI，而是改为对公网地址做真实 HTTP/API 自动验收。已确认 `POST /api/auth/login` 可用，`zhangsan_sales / Sales@123` 能成功获取 `accessToken`，随后 `GET /api/auth/me` 返回销售角色与有效菜单 / 动作权限集合；进一步使用同一令牌验收 `GET /api/opportunities/15` 与 `GET /api/opportunities/7`，确认云上真实 MySQL 数据集中同时存在“审批已闭环”与“审批进行中”两类商机样本：其中 `opportunities.id=15` 返回 `approvalStatus=approved`、`bizApprovalStatus=approved`、`techApprovalStatus=approved`、`researchDocName=轨交运维协同平台_调研纪要_v1.0.docx`、`approvalOpinion=业务、技术审批已闭环，转入商务谈判与合同准备。`；`opportunities.id=7` 返回三类审批状态均为 `pending` 且仍保留 `currentApprovalInstanceId=9`。需要单独说明的是：云端当前使用真实 MySQL 数据集，其样本商机主键与本地 SQLite 验收用例不一致，因此本地终态样本 `OPP-000001 / opportunities.id=1` 并不在云上作为同一条记录存在；这属于数据口径差异，不是本次代码发布回退。
- 浏览器自动化恢复补记（确认日期：2026-04-09）：随后已继续排查本机 Playwright 浏览器问题，确认根因不是页面或脚本本身，而是 Codex 受限执行环境对本机 Chrome `crashpad` 的限制。在沙箱内启动 `Playwright + /Applications/Google Chrome.app/.../Google Chrome` 会报 `bootstrap_check_in ... Permission denied (1100)` 并触发 `SIGABRT`；改为非沙箱模式执行同一启动链路后，Chrome 可正常 headless 启动，说明浏览器二进制本身可用。基于此，当前公网 UI 回归应按“非沙箱执行浏览器脚本”的口径运行。
- 浏览器自动化恢复补记（确认日期：2026-04-09）：现有 [output/playwright/opportunity-regression-check.mjs](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/opportunity-regression-check.mjs) 已补充 `FRONTEND_URL` 与一组可选断言环境变量（`TARGET_OPPORTUNITY_CODE`、`EXPECTED_OWNER_TEXT`、`EXPECTED_RESEARCH_DOC_TEXT`、`EXPECTED_FINAL_COMMENT_TEXT` 等），以兼容“本地 SQLite 样例”和“云上 MySQL 真实数据”两种不同数据口径。随后已在非沙箱模式下对公网地址重新执行脚本，命令口径为：`FRONTEND_URL=http://101.43.78.27 TARGET_OPPORTUNITY_CODE=OPP-000016 EXPECTED_OWNER_TEXT='当前处理人：示例售前工程师' EXPECTED_RESEARCH_DOC_TEXT='当前文档：城市生命线监测_调研纪要_v4.0.docx' EXPECTED_FINAL_COMMENT_TEXT='最终审批 · 最终审批通过' node output/playwright/opportunity-regression-check.mjs`；最新 `opportunity-regression-report.json` 中 `loginSucceeded / navigatedToOpportunities / approvalModalOpened / opportunityListShowsApproved / approvalModalShowsCompletedFlow / approvalModalShowsOwnerAndResearchDoc / approvalModalShowsFinalComment` 已全部为 `true`，控制台仅剩登录页密码框的 `autocomplete` 提示，无新的页面异常与请求失败。
- 深色模式滚动条收口补记（确认日期：2026-04-09）：已确认当前“深色模式下部分右侧滚动条仍呈浅色”的根因，不是单页样式遗漏，而是项目此前只在 `settings-view` 局部做过 `scrollbar-color` 覆盖，导致系统设置的部分目录列表更接近目标态，但工作台待办区、知识库分类区、审批流程编辑区以及应用主内容区仍会回退到浏览器默认浅色滚动条。当前已在 [frontend/src/theme.css](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/theme.css) 新增统一的 `app-scrollbar` 滚动条样式层：浅色模式下保持较轻的半透明灰色滑块，深色模式下切换为深灰蓝滑块与弱存在轨道，以保证不破坏浅色模式既有表现，同时让深色模式下的滚动条视觉与“知识库目录管理 > 一级知识库列表”保持一致方向。
- 深色模式滚动条收口补记（确认日期：2026-04-09）：本轮已将统一滚动条样式接入以下关键滚动容器：应用主内容区与左侧主菜单滚动区 [frontend/src/components/AppAuthenticatedShell.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/components/AppAuthenticatedShell.tsx)，工作台“待办任务”右侧滚动区 [frontend/src/views/WorkbenchView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/WorkbenchView.tsx)，知识库分类右侧滚动区 [frontend/src/views/KnowledgeView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/KnowledgeView.tsx)，系统设置中的“一级知识库列表 / 二级子分类列表” [frontend/src/views/settings/SettingsExtendedPanels.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/SettingsExtendedPanels.tsx)，以及审批流程编辑弹窗节点列表 [frontend/src/views/settings/WorkflowEditorModal.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/WorkflowEditorModal.tsx)。已重新执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建通过；当前这一步属于样式统一收口，后续仍建议基于浏览器逐页复查项目管理、商机管理、解决方案、投标管理、合同管理、知识库和系统设置中所有可滚动子面板，确认是否还有未挂上统一滚动条类名的局部容器。
- 深色模式滚动条验收补记（确认日期：2026-04-09）：当前已补充一份专用浏览器验收脚本 [output/playwright/scrollbar-theme-check.mjs](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/scrollbar-theme-check.mjs)，用于登录管理员账号、切换深色模式、逐页进入 `工作台 / 项目管理 / 商机管理 / 解决方案 / 投标管理 / 合同管理 / 知识库 / 系统设置 > 团队管理 / 系统设置 > 知识库目录管理`，并对页面内 `.app-scrollbar` 容器做滚动条加宽后截图，便于自动验收与人工复核。该脚本当前默认兼容本地 `http://127.0.0.1:5173` 与云端 `http://101.43.78.27` 两种入口。
- 深色模式滚动条验收补记（确认日期：2026-04-09）：已在本地运行中的前后端服务上，以 `admin_demo / Admin@123` 执行 `FRONTEND_URL=http://127.0.0.1:5173 OUTPUT_PREFIX=local node output/playwright/scrollbar-theme-check.mjs`；最新 [output/playwright/local-scrollbar-summary.json](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/local-scrollbar-summary.json) 显示 9 个目标页全部处于 `theme=dark`，且每页都已检测到 2-4 个 `.app-scrollbar` 容器，`includesMenuText` 均为 `true`。同时已产出本地截图 [local-workbench.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/local-workbench.png)、[local-settings-knowledge-category.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/local-settings-knowledge-category.png) 等文件，人工抽查确认工作台待办区和“知识库目录管理 > 一级知识库列表”中的滚动条均为深色风格，未出现浅色滚动条回退。
- 深色模式滚动条验收补记（确认日期：2026-04-09）：在本地验证通过后，已按最小变更范围将 `README.md` 与 6 个前端源码文件打包同步到 `deploy@101.43.78.27:/opt/presales-platform`，并仅执行 `docker compose build frontend && docker compose up -d frontend`，未重建 MySQL 与 backend。随后再次以同一管理员账号执行 `FRONTEND_URL=http://101.43.78.27 OUTPUT_PREFIX=cloud node output/playwright/scrollbar-theme-check.mjs`；最新 [output/playwright/cloud-scrollbar-summary.json](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud-scrollbar-summary.json) 与本地结果一致，9 个目标页均为深色模式且滚动容器计数正常。云端截图 [cloud-workbench.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud-workbench.png) 与 [cloud-settings-knowledge-category.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud-settings-knowledge-category.png) 抽查结果也与本地一致，说明本轮滚动条样式收口已在云环境完成发布和浏览器级验证。
- 深色模式滚动条补漏补记（确认日期：2026-04-09）：在第一轮收口后继续对前端源码扫描，当前已确认内联样式层面剩余未挂上统一滚动条类名的滚动容器只剩 [frontend/src/views/analytics/AnalyticsDashboardPreview.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/analytics/AnalyticsDashboardPreview.tsx) 的预览面板滚动区。现已为该容器补上 `app-scrollbar`，从而把数据分析自定义仪表盘预览的滚动条也纳入与主应用一致的深浅主题样式体系，避免该预览区域在深色模式下回退到浏览器默认浅色滚动条。
- 深色模式滚动条补漏补记（确认日期：2026-04-09）：补漏后已再次执行本地构建 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建继续通过；随后以同一脚本重新执行本地与云上二次浏览器自动化验收，分别生成 [output/playwright/local2-scrollbar-summary.json](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/local2-scrollbar-summary.json) 与 [output/playwright/cloud2-scrollbar-summary.json](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud2-scrollbar-summary.json)。两份摘要结果与第一轮一致，9 个目标页全部保持 `theme=dark`、`includesMenuText=true` 且滚动容器计数稳定，说明补齐数据分析预览滚动区后未对既有页面造成视觉或结构回退。至此，当前源码中已显式声明的主要 `overflowY/overflow: auto` 滚动容器均已接入统一的 `app-scrollbar` 主题样式。
- 深色模式滚动条三次补漏补记（确认日期：2026-04-10）：继续按源码扫描 `overflowX/overflowY/overflow: auto` 容器后，新增发现 3 处此前未挂统一滚动条类名的横向滚动区域，分别位于 [frontend/src/views/settings/MenuPermissionsPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/MenuPermissionsPanel.tsx)、[frontend/src/views/settings/ActionPermissionsPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/ActionPermissionsPanel.tsx) 和 [frontend/src/views/feishu/FeishuCardsPreviewPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/feishu/FeishuCardsPreviewPanel.tsx)。当前已统一补上 `app-scrollbar`，使“菜单权限/操作权限成员横向选择条”以及“飞书卡片动作 JSON 预览区”在深色模式下不再回退到浏览器默认浅色滚动条。
- 深色模式滚动条三次补漏补记（确认日期：2026-04-10）：补漏后已重新执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建通过。当前这一步仍属于局部视觉收口，尚未新增浏览器验收截图；若后续继续发布到云端，可直接把这 3 个文件与前序滚动条收口改动一并纳入最小前端发布包。
- 深色模式滚动条本地三次验收补记（确认日期：2026-04-11）：本轮继续执行第 3 次补漏对应的浏览器验收时，先尝试复用现有 [output/playwright/scrollbar-theme-check.mjs](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/scrollbar-theme-check.mjs)，但脚本当前硬编码依赖 `/private/tmp/pwcheck/node_modules/playwright/index.mjs`，在本机该路径已不存在，直接运行会报 `ERR_MODULE_NOT_FOUND`。由于这属于验收脚本运行环境漂移，而不是前端页面本身异常，本轮改为使用 Playwright MCP 直接连本地前后端服务完成同等验证。
- 深色模式滚动条本地三次验收补记（确认日期：2026-04-11）：已在本地重新启动 `backend` 与 `frontend` 服务，并用管理员账号 `admin_demo / Admin@123` 登录后切到深色模式，逐一验证“系统设置 > 权限管理中心 > 菜单权限”、“系统设置 > 权限管理中心 > 操作权限”以及“系统设置 > 飞书集成 > 卡片预览”三个新增命中区域。结果已汇总到 [local3-scrollbar-manual-summary.json](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/local3-scrollbar-manual-summary.json)，并补充截图 [local3-settings-menu-permissions.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/local3-settings-menu-permissions.png)、[local3-settings-action-permissions.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/local3-settings-action-permissions.png) 与 [local3-settings-feishu-cards.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/local3-settings-feishu-cards.png)。
- 深色模式滚动条本地三次验收补记（确认日期：2026-04-11）：两处权限中心页面都已在深色模式下命中 1 个横向 `.app-scrollbar` 成员选择条；飞书集成页的动作载荷 JSON 预览容器也已确认挂载 `.app-scrollbar`，只是当前默认视口下该 JSON 片段未形成横向溢出，因此截图中不会稳定出现横向滚动条轨道。控制台未发现新的页面崩溃或未捕获异常；当前残留信息仍主要是登录前探测 `/auth/me` 产生的 401，以及若干历史存在的 Ant Design deprecation warning。
- Playwright 验收脚本运行时修复补记（确认日期：2026-04-11）：针对 [output/playwright/scrollbar-theme-check.mjs](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/scrollbar-theme-check.mjs) 与 [output/playwright/opportunity-regression-check.mjs](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/opportunity-regression-check.mjs) 仍硬编码 `/private/tmp/pwcheck/node_modules/playwright/index.mjs` 的问题，当前已新增共享运行时 [playwright-runtime.mjs](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/playwright-runtime.mjs)，统一改为按以下顺序解析 Playwright：`PLAYWRIGHT_MODULE_PATH` 环境变量 -> 当前 Node 运行环境中的本地 `playwright` 依赖 -> `~/.npm/_npx/*/node_modules/playwright` 缓存 -> 最后才回退到 `npx --package playwright` 动态解析。
- Playwright 验收脚本运行时修复补记（确认日期：2026-04-11）：修复后已执行 `node --input-type=module -e "import { loadPlaywright } from './output/playwright/playwright-runtime.mjs'; const mod = await loadPlaywright(); console.log(Object.keys(mod).includes('chromium') ? 'chromium-ok' : 'chromium-missing');"`，当前返回 `chromium-ok`，说明脚本运行时已可在本机稳定解析 Playwright 模块，不再绑定单次会话生成的临时目录。
- Playwright 验收脚本自动化恢复补记（确认日期：2026-04-11）：在运行时修复后，已重新启动本地 `backend` 与 `frontend` 服务，并成功执行 `FRONTEND_URL=http://127.0.0.1:5173 OUTPUT_PREFIX=local3 node output/playwright/scrollbar-theme-check.mjs`。最新 [local3-scrollbar-summary.json](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/local3-scrollbar-summary.json) 已不再含 `fatalError`，说明滚动条验收脚本自身已恢复为可直接自动化执行，不再需要依赖 MCP 人工浏览器兜底。
- Playwright 验收脚本自动化恢复补记（确认日期：2026-04-11）：本次自动化结果已覆盖新增的 `settings-menu-permissions`、`settings-action-permissions` 与 `settings-feishu-cards` 三个目标页，并确认 `includesEnsureTexts` 全部为 `true`。其中“菜单权限管理”“操作权限管理”两页的 `horizontalScrollerCount` 均为 `1`，与前序人工验收一致；“飞书集成 > 卡片预览”页 `horizontalScrollerCount=0`，但 `scrollerCount=3` 且目标文本命中正常，符合“JSON 预览容器已挂 `app-scrollbar`、只是当前视口未形成横向溢出”的预期。
- Playwright 产物目录收口补记（确认日期：2026-04-11）：当前已对 [output/playwright](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright) 做一轮基线清理，删除已被替代的 `local*` / `local2*` / 第一轮 `cloud*` 滚动条验收产物，以及本次排障期间产生的 MCP 临时快照和 `local3` 人工兜底摘要，只保留最新的本地自动化基线 `local3-*`、最新云端自动化基线 `cloud2-*`、商机审批回归快照 `01-* ~ 04-*` 和脚本本身。目录使用约定已补充到 [output/playwright/README.md](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/README.md)。
- 云端滚动条扩展验收补记（确认日期：2026-04-11）：本轮已使用密码方式连接 `deploy@101.43.78.27`，按最小变更口径仅同步 10 个前端源码文件到 `/opt/presales-platform/frontend/src/...`，随后在云端执行 `docker compose build frontend && docker compose up -d frontend` 完成前端单容器重建；`docker compose ps frontend` 显示容器已恢复运行，`curl http://127.0.0.1/` 也已返回新的前端入口 HTML。
- 云端滚动条扩展验收补记（确认日期：2026-04-11）：发布后已重新执行 `FRONTEND_URL=http://101.43.78.27 OUTPUT_PREFIX=cloud3 node output/playwright/scrollbar-theme-check.mjs`。最新 [cloud3-scrollbar-summary.json](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud3-scrollbar-summary.json) 显示 12 个目标页全部为 `theme=dark`、`includesEnsureTexts=true`；其中 `settings-menu-permissions` 与 `settings-action-permissions` 的 `horizontalScrollerCount` 均为 `1`，`settings-feishu-cards` 为 `0` 且 `scrollerCount=3`，与本地 `local3` 自动化结果完全一致。由于 `cloud3-*` 已覆盖并超集替代上一轮 `cloud2-*`，当前已删除旧的 `cloud2-*` 云端基线产物，并将 [output/playwright/README.md](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/README.md) 的保留口径更新为“本地保留 `local3-*`，云端保留 `cloud3-*`”。
- 本次会话未继续扩大代码改动面，也未执行云端同步、容器重建或 Git 合并；当前更适合把今天这次“继续验证 + 最新数值”作为新的文档基线，后续若继续性能线，再围绕 `ECharts` 依赖图和 `antd` 依赖桶做下一轮定向收口。
- 前端性能方向 1 补记（确认日期：2026-04-11）：本轮已按“代码级延迟加载优先、`manualChunks` 仅保留稳定基线”的口径继续收口前端构建。此前针对 `antd` / `rc-*` 的进一步手工拆包实验已开始引入 circular chunk warning，当前已将 [frontend/vite.config.ts](/Users/gjy/Projects-mygetpre/presales-platform/frontend/vite.config.ts) 回退到上一版稳定策略，只保留 `react`、`antd`、`echarts` 与 `echarts-gl` 等公共依赖的大类拆分，不再继续用更细颗粒的 `manualChunks` 作为主优化方向。
- 前端性能方向 1 补记（确认日期：2026-04-11）：数据分析页已将 [EChartsPreview.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/components/EChartsPreview.tsx) 从 [AnalyticsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/AnalyticsView.tsx) 的同步依赖改为 `React.lazy + Suspense` 按需加载，并为漏斗图、趋势图、行业分布、甘特图四个标准面板补上统一的图表占位骨架。当前 `AnalyticsView` 主 chunk 已不再静态绑定图表运行时入口，只有进入页面并真正渲染对应图表区域时才会请求独立的 `EChartsPreview-*.js` 与细分 `vendor-echarts-*` 运行时块。
- 前端性能方向 1 补记（确认日期：2026-04-11）：飞书集成页已新增 [FeishuBindingManagementPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/feishu/FeishuBindingManagementPanel.tsx)，并在 [FeishuIntegrationView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/FeishuIntegrationView.tsx) 中将左侧“绑定管理（Mock / API）”整体改为异步加载。这样主页面初始只保留状态、筛选和回调入口，实际的表单、筛选控件与新增绑定 UI 在进入该区域时才下载对应 chunk，避免飞书集成页把低频编辑表单与右侧预览区一起同步打进首包。
- 前端性能方向 1 补记（确认日期：2026-04-11）：已重新执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建通过且未再出现新增的 circular chunk warning。最新产物中 [AnalyticsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/AnalyticsView.tsx) 对应 chunk 约 `28.27 kB`、[FeishuIntegrationView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/FeishuIntegrationView.tsx) 约 `21.15 kB`，并新增独立 `EChartsPreview-*.js`（约 `3.34 kB`）与 `FeishuBindingManagementPanel-*.js`（约 `2.15 kB`）。当前构建阶段剩余的大块主要集中在 `vendor-antd`（约 `514.95 kB`）、`vendor-echarts-runtime`（约 `555.85 kB`）和 `vendor-echarts-gl`（约 `599.96 kB`）这类公共依赖桶，说明下一轮若继续性能线，应优先继续下沉“页面内低频重模块”或评估 `echarts-gl` 的真实使用边界，而不是重新回到高风险的人工细拆包。
- 前端性能方向 1 补记（确认日期：2026-04-11）：本轮继续对 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 做页面内异步收口，已将默认显示的“审批流程库”主面板抽成独立的 [SettingsWorkflowPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/SettingsWorkflowPanel.tsx)，并改为 `React.lazy + Suspense` 按需加载。当前系统设置页主文件只保留流程列表状态、权限判断和回调入口，实际的流程表格、默认流程标识和行内操作按钮在进入该面板时再请求独立 chunk。
- 前端性能方向 1 补记（确认日期：2026-04-11）：抽离审批流程库面板后已再次执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建继续通过。最新产物中新增 `SettingsWorkflowPanel-*.js`（约 `3.33 kB`），而 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 对应 chunk 已从约 `62.75 kB` 进一步下降到约 `60.74 kB`。这说明“系统设置页继续把低频的大块 JSX 与表格视图下沉到异步边界”仍然是有效方向；下一轮若继续推进，优先候选会是团队管理主面板或其他仍保留在 `SettingsView` 内部的大体量同步区块。
- 前端性能方向 1 补记（确认日期：2026-04-11）：在继续沿用同一路径后，当前又已将 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 中“团队与权限管理”主面板抽成独立的 [SettingsTeamPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/SettingsTeamPanel.tsx)，并通过 `React.lazy + Suspense` 按需加载。现在成员筛选器、列表表格、列设置弹层入口和角色权限说明卡片都会在进入团队管理菜单时再加载，而不是继续占用系统设置主包。
- 前端性能方向 1 补记（确认日期：2026-04-11）：抽离团队管理面板后已再次执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建继续通过。最新产物中新增 `SettingsTeamPanel-*.js`（约 `3.96 kB`），而 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 对应 chunk 已从约 `60.74 kB` 继续下降到约 `58.25 kB`。结合前一轮 `SettingsWorkflowPanel-*.js`（约 `3.33 kB`）的拆分结果，当前系统设置页的进一步减重路径已经比较明确：优先把只有进入对应菜单时才会使用的大块列表/表格/说明区继续保留在异步边界，而不是重新回到手工细拆 `vendor` 包。
- 前端性能方向 1 补记（确认日期：2026-04-11）：本轮继续把 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 中“已拆子面板、但父层运行时仍留在主包”的权限中心收口为独立异步边界。当前已新增 [SettingsPermissionsCenterPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/SettingsPermissionsCenterPanel.tsx)，并将菜单权限/操作权限两套 state、definitions 加载、用户权限详情加载、草稿派生、模块分组、保存/恢复/全选/清空等运行时逻辑一并下沉进去；`SettingsView` 只保留 `permissionsCenter` 的 lazy 入口与最小必要 props。
- 前端性能方向 1 补记（确认日期：2026-04-11）：收口权限中心运行时后已再次执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建继续通过。最新产物中新增 `SettingsPermissionsCenterPanel-*.js`（约 `15.15 kB`），而 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 对应 chunk 已从约 `58.25 kB` 明显下降到约 `44.32 kB`。这说明当前对系统设置页最有效的方向确实不是继续细拆公共依赖，而是把“完整子系统级运行时”整体下沉到异步边界。
- 前端性能方向 1 补记（确认日期：2026-04-11）：在权限中心之外，当前又已把 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 中“个人资料 / 安全设置”两块同步表单视图抽成独立的 [SettingsAccountPanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/SettingsAccountPanel.tsx)，并通过 `React.lazy + Suspense` 按需加载。当前系统设置主文件只保留账号资料与密码更新所需的最小状态、表单实例和提交回调，实际的资料表单、账号概览卡片和密码修改表单都在进入对应菜单时再请求独立 chunk。
- 前端性能方向 1 补记（确认日期：2026-04-11）：抽离账号资料/安全设置面板后已再次执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建继续通过。最新产物中新增 `SettingsAccountPanel-*.js`（约 `5.37 kB`），而 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 对应 chunk 已从约 `44.32 kB` 继续下降到约 `40.18 kB`。这说明系统设置页剩余的同步体积已经进一步集中到团队管理、知识库目录与少量菜单壳层/共享状态装配，后续若继续沿“方向1”推进，优先级会落在这些剩余运行时上，而不是再回到 bundler 细拆。
- 前端性能方向 1 补记（确认日期：2026-04-11）：沿同一路径继续推进后，当前又已把 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 中“知识库目录管理”的父层运行时抽成独立的 [SettingsKnowledgeRuntimePanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/SettingsKnowledgeRuntimePanel.tsx)。现在知识库目录树的本地初始化、后端加载、导入/导出、排序移动、分组增删改、重置与本地存储同步等逻辑都已下沉到新的异步面板中；`SettingsView` 只保留 lazy 入口和初始化目录数据。
- 前端性能方向 1 补记（确认日期：2026-04-11）：抽离知识库目录运行时后已再次执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建继续通过。最新产物中新增 `SettingsKnowledgeRuntimePanel-*.js`（约 `5.66 kB`），而 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 对应 chunk 已从约 `40.18 kB` 继续下降到约 `35.94 kB`。至此，系统设置页的大块同步子系统已经进一步收口，后续若仍沿“方向1”推进，最值得继续看的会是团队管理剩余表格运行时，以及 `echarts-gl` 自定义仪表盘链路的真实 3D 预览边界。 
- 前端性能方向 1 补记（确认日期：2026-04-11）：本轮继续收口 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 中团队管理剩余的父层运行时，已新增 [SettingsTeamRuntimePanel.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/SettingsTeamRuntimePanel.tsx)，并将成员筛选、分页、列显示配置、本地持久化、成员增删改/启停、随机密码生成以及团队弹窗挂载整体下沉到新的异步边界中；`SettingsView` 只保留团队管理的 lazy 入口，同时把权限中心成员列表切换为独立的轻量加载路径，不再继续绑定团队页自己的筛选运行时。
- 前端性能方向 1 补记（确认日期：2026-04-11）：抽离团队管理运行时后已再次执行 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build`，构建继续通过。最新产物中新增 `SettingsTeamRuntimePanel-*.js`（约 `16.58 kB`）与保留的 `TeamMemberModals-*.js`（约 `3.63 kB`），而 [SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 对应 chunk 已从约 `35.94 kB` 进一步下降到约 `26.46 kB`。当前系统设置页主包已基本只剩菜单壳层、流程库状态装配、权限中心入口与少量共享样式；下一轮若仍沿“方向1”推进，更值得转向 `AppAuthenticatedShell` 低收益异步点复核，或继续回到 `echarts-gl` 真 3D 触发链路做精准裁剪。 
- 云端发布补记（确认日期：2026-04-11）：已按“最小上传包”口径将 [frontend/src/views/SettingsView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/SettingsView.tsx) 与整个 [frontend/src/views/settings/](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/settings/) 目录同步到 `deploy@101.43.78.27:/opt/presales-platform`。首次仅同步两份文件时，云端 `frontend` 镜像构建报错缺少 `./settings/SettingsWorkflowPanel`，据此确认服务器上的设置页拆分基线落后于本地；随后补齐整个 settings 子目录后再次执行 `docker compose build frontend && docker compose up -d frontend`，云端前端构建与容器重建均已恢复成功。
- 云端自动化验收补记（确认日期：2026-04-11）：发布后已通过浏览器自动化重新登录公网入口 `http://101.43.78.27/`，并逐项验证“系统设置 > 团队管理”、“系统设置 > 权限管理中心 > 菜单权限”以及“系统设置 > 权限管理中心 > 操作权限”三条关键路径。当前已确认团队管理页能正常展示真实成员数据、`+ 添加成员 / 列设置 / 权限说明` 区块及 `查看 / 编辑 / 禁用 / 删除` 操作列；权限中心也能正常展示成员横向单选条、菜单权限矩阵、操作权限矩阵以及 `team.manage / menu-permission.manage / action-permission.manage` 等设置动作项。验收结果已记录到 [cloud4-settings-validation.json](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud4-settings-validation.json)，并补充截图 [cloud4-settings-team.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud4-settings-team.png)、[cloud4-settings-menu-permissions.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud4-settings-menu-permissions.png) 与 [cloud4-settings-action-permissions.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud4-settings-action-permissions.png)。
- 飞书 / OpenClaw 联调补记（确认日期：2026-04-11）：本轮继续沿“先把现有后端能力真正暴露给平台页”的思路推进。后端已在 [openclaw.controller.ts](/Users/gjy/Projects-mygetpre/presales-platform/backend/src/integrations/openclaw/openclaw.controller.ts) 中新增浏览器专用 JWT 接口 `GET /integrations/openclaw/playground/skills` 与 `POST /integrations/openclaw/playground/skills/:name`，用于在不暴露 `x-openclaw-token` 共享密钥的前提下，把现有 4 个只读 skill 目录和结构化执行能力安全开放给登录态页面；原有服务间接口 `skills / skills/:name / query` 与浏览器自然语言入口 `playground/query` 保持不变。
- 飞书 / OpenClaw 联调补记（确认日期：2026-04-11）：前端 [OpenClawPlaygroundView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpenClawPlaygroundView.tsx) 已同步接入上述浏览器专用 skill 接口。当前“系统设置 -> OpenClaw联调”除自然语言测试外，又新增“Skill 模式”结构化执行区：可加载技能目录、切换 `get_my_pending_approvals / get_opportunity_summary / get_solution_summary / get_daily_brief` 四个只读 skill，并按各自 schema 填写 `code / limit / businessType` 等最小参数直接执行；执行结果继续复用现有结果区、会话概览、请求历史和调试区，从而同时覆盖“意图路由”和“结构化 skill 契约”两条联调路径。
- 飞书 / OpenClaw 联调补记（确认日期：2026-04-11）：[FeishuIntegrationView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/FeishuIntegrationView.tsx) 也已同步完成一轮口径收口。页面顶部已不再继续标注“当前页面为前端原型 / 后端接口待接入”，而是明确为“平台页联调 + 后端能力验收”；左侧绑定管理标题更新为“API 优先 / Mock 回退”，并新增“审批卡片联调”卡片，可直接调用 `POST /integrations/feishu/messages/approval-cards/send` 以 `approvalInstanceId + bindingId/openId` 做 `dryRun` 预览或真实发送，从平台页直接验证审批卡片出站链路。
- 飞书 / OpenClaw 联调补记（确认日期：2026-04-11）：本轮变更已再次执行本地构建验证：`cd /Users/gjy/Projects-mygetpre/presales-platform/backend && npm run build` 与 `cd /Users/gjy/Projects-mygetpre/presales-platform/frontend && npm run build` 均已通过。当前下一步待继续完成的是将这批飞书 / OpenClaw 联调增强同步到云端，并在公网环境补一轮浏览器自动化验收，重点覆盖“OpenClaw Skill 模式”和“飞书审批卡片联调”两条新路径。
- 云端发布补记（确认日期：2026-04-11）：本轮已将 [backend/src/integrations/openclaw/openclaw.controller.ts](/Users/gjy/Projects-mygetpre/presales-platform/backend/src/integrations/openclaw/openclaw.controller.ts)、[frontend/src/views/OpenClawPlaygroundView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/OpenClawPlaygroundView.tsx)、[frontend/src/views/openclaw-playground/](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/openclaw-playground/) 与 [frontend/src/views/FeishuIntegrationView.tsx](/Users/gjy/Projects-mygetpre/presales-platform/frontend/src/views/FeishuIntegrationView.tsx) 等本轮改动最小包同步到 `deploy@101.43.78.27:/opt/presales-platform`，随后在服务器执行 `docker compose build backend frontend && docker compose up -d backend frontend`。重建完成后已确认 `http://127.0.0.1/api/health` 返回 `{\"status\":\"ok\"}`、`http://127.0.0.1/` 返回 `HTTP/1.1 200 OK`，说明本轮 OpenClaw 后端浏览器接口与飞书前端联调页均已成功上线。
- 云端自动化验收补记（确认日期：2026-04-11）：发布后已再次登录公网入口 `http://101.43.78.27/` 并补充两条新路径自动化回归。其一，在“系统设置 > OpenClaw联调”页面已确认出现 `Skill 模式 / 结构化技能执行 / 执行 Skill` 新区块，技能目录显示 `4 个只读 skill`；直接执行 `get_daily_brief` 后，结果区成功返回 `playground_skill_execution`、`requestId`、`日期=2026-04-11`、`待审批=3`、`风险商机=3` 与 `方案更新=10`，说明浏览器专用 skill 执行链路已在线生效。其二，在“系统设置 > 飞书集成 > 审批卡片联调”页面已确认出现“平台页联调 / 只读接口已接通 / 审批卡片可预演”新文案和发送表单；进一步以 `approvalInstanceId=20`、`bindingId=1` 执行 `dryRun`，页面成功返回 `Dry Run 预览 · dry_run`，并展示接收人 `ou_04f5c38ef1840e7fe1fbdadd57b62e06` 与卡片标题“方案审批：SOL-000006 连锁零售数据中台建设方案”，说明审批卡片平台页联调入口也已在线可用。验收结果已记录到 [cloud5-feishu-openclaw-validation.json](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud5-feishu-openclaw-validation.json)，并补充截图 [cloud5-openclaw-skill-mode.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud5-openclaw-skill-mode.png) 与 [cloud5-feishu-approval-dryrun.png](/Users/gjy/Projects-mygetpre/presales-platform/output/playwright/cloud5-feishu-approval-dryrun.png)。
