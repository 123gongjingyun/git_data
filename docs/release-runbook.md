...
- 优先完成代码修改、构建验证、文档更新
- 优先运行浏览器验收与前端细节调整
- GitHub 访问需与本机当前代理保持一致；当前已确认可用代理端口为 `127.0.0.1:7890`
...
如需同步 GitHub：
- 若 `git fetch` / `git push` 长时间无返回，优先检查本机代理是否走 `127.0.0.1:7890`；
- 仅在确认代理连通后再判断是否为凭据或远端权限问题。
...
- 上传包只包含最小必要文件
- 当前改动已经明确是否需要云端重建镜像
- 若需推送 GitHub，已确认本机代理口径与 GitHub 联通口径一致（当前为 `127.0.0.1:7890`）
